...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the official 2001 season-index capture.")
    ...
    if not args.validate_only:
        ...
        mutations.append(
            expect_rejection(
                "invented_season_url",
                lambda: _validate(
                    _mutated_gate(gate, official_index_url="https://files.12thman.com/history/football/years/2002.html")
                ),
            )
        )
        ...
        invented = list(gate["box_score_urls"]) + [
            "https://files.12thman.com/history/football/stats/2001-2002/invented.html"
        ]
        ...
        mutations.append(
            expect_rejection("reordered_box_urls", lambda: _validate(_mutated_gate(gate, box_score_urls=reordered_urls)))
        )
        ...
        mutations.append(
            expect_rejection(
                ...
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream_609)),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                ...
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream_613)),
            )
        )
        mutations.append(
            expect_rejection(
                "stale_code_identity",
                lambda: _validate(_mutated_gate(gate, validator_code_identity="0" * 64)),
            )
        )
