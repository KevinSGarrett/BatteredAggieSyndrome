...
class OpenAIAssistTests(unittest.TestCase):
    ...
    def test_depth_chart_pilot_preflight_includes_meaningful_terra_and_sol_without_pit_authority(self):
        ...
        self.assertEqual(56, report["execution_attempt"]["completed_predictions"])
        self.assertEqual(14, report["execution_attempt"]["terra_calls"])
        self.assertEqual(14, report["execution_attempt"]["sol_calls"])
        empirical = report["empirical_comparison"]
        self.assertEqual("FAIL", empirical["routes"]["gpt-5-nano:minimal"]["gate"])
        self.assertEqual("PASS", empirical["routes"]["gpt-5.6-luna:none"]["gate"])
        self.assertEqual("PASS", empirical["routes"]["gpt-5.6-terra:low"]["gate"])
        self.assertEqual("PASS", empirical["routes"]["gpt-5.6-sol:medium"]["gate"])
        self.assertIn("LUNA_CHEAPEST_PASSING", empirical["route_decision"])
        self.assertTrue(empirical["batch_status"].startswith("NOT_SUBMITTED"))
        self.assertTrue(report["credential_checkpoint"]["process_environment_key_used"])
