...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate official 2003 structured domains.")
    ...
    if not args.validate_only:
        ...
        mutations.append(expect_rejection("participation_as_availability", lambda: _validate(_mutated_gate(gate, authority=authority))))
        ...
        mutations.append(expect_rejection("coverage_without_rows", lambda: _validate(_mutated_gate(gate, counts=counts3))))
        upstream = json.loads(json.dumps(gate["upstream_identities"]))
        upstream["bat610_gate_identity"] = "0" * 64
        mutations.append(expect_rejection("bat610_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream))))
        upstream = json.loads(json.dumps(gate["upstream_identities"]))
        upstream["bat601_gate_identity"] = "0" * 64
        mutations.append(expect_rejection("bat601_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream))))
        upstream2 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream2["html_table_classifier_identity"] = "forged.classifier.v9"
        mutations.append(expect_rejection("classifier_identity_mutated", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream2))))
        mutations.append(
            expect_rejection(
