...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the governed shadow entity-review pilot")
    parser.add_argument("--gold", type=Path, required=True)
    parser.add_argument("--max-jobs", type=int, default=42)
    ...
    if hashlib.sha256(prompt_bytes).hexdigest() != prompt_spec["sha256"]:
        raise SystemExit("entity-review pilot prompt hash mismatch")
    prompt_base = prompt_bytes.decode("utf-8").rstrip()
    routes = config["routes"]
    job_count = len(gold) * len(routes)
    if job_count == 0 or job_count > args.max_jobs:
        raise SystemExit(f"bounded entity-review job count {job_count} is outside 1..{args.max_jobs}")
    ...
    for route in routes:
        for case in gold:
            ...
            prompt = (
                f"{prompt_base}\n\nCase instruction: copy case_id {case['case_id']}. "
                f"Return exactly these facts in order: {json.dumps(fields, separators=(',', ':'))}. "
                f"The allowed candidate IDs are {json.dumps(case['allowed_candidate_ids'], separators=(',', ':'))}."
            )
            job = AssistiveJob(
            ...
            try:
                ...
                for key in ["input_tokens", "cached_input_tokens", "output_tokens", "reasoning_tokens"]:
                    tokens[f"{route['model']}:{key}"] += int(usage[key])
                if candidate is None:
                    failures.append({"model": route["model"], "case_id": case["case_id"], "error": "NO_CANDIDATE", "disposition": result.disposition, "validation_errors": list(result.validation_errors)})
                    continue
                top_k = candidate.get("entity_top_k", [])
                candidate_set_errors: list[str] = []
                if len(top_k) != len(set(top_k)):
                    candidate_set_errors.append("DUPLICATE_ENTITY_TOP_K")
                if set(top_k) != set(case["allowed_candidate_ids"]):
                    candidate_set_errors.append("ENTITY_TOP_K_SET_MISMATCH")
                if top_k != case["expected_entity_top_k"]:
                    candidate_set_errors.append("ENTITY_TOP_K_ORDER_MISMATCH")
                if candidate.get("entity_merge") is not False:
                    candidate_set_errors.append("ENTITY_MERGE_AUTHORITY_VIOLATION")
                rows.append({"case_id": case["case_id"], "model": route["model"], "reasoning_effort": route["reasoning_effort"], "actual_cost_usd": result.actual_cost_usd, "review_time_saved_seconds": 0, "disposition": result.disposition, "validation_errors": [*result.validation_errors, *candidate_set_errors], "entity_merge": candidate.get("entity_merge"), "entity_top_k": top_k, "candidate": candidate})
    ...
    predictions = controller.store.put_bytes("evals", payload, suffix=".entity-review-predictions.jsonl")
    manifest = controller.store.put_json("evals", {"schema_version": 1, "artifact_type": "openai_entity_review_pilot_run", "pilot_id": config["pilot_id"], "jira_unit": config["jira_unit"], "authority": config["authority"], "prompt_version": prompt_spec["version"], "prompt_sha256": prompt_spec["sha256"], "gold_sha256": hashlib.sha256(gold_path.read_bytes()).hexdigest(), "case_count": len(gold), "requested_jobs": job_count, "completed_predictions": len(rows), "failures": failures, "predictions_sha256": predictions.sha256, "predictions_bytes": predictions.bytes, "cost_usd_by_model": {key: f"{value:.6f}" for key, value in sorted(costs.items())}, "tokens_by_model": dict(sorted(tokens.items())), "budget_after_run": controller.ledger.summary(), "final_disposition": "SHADOW_REVIEW_ONLY"})
    print(json.dumps({"requested_jobs": job_count, "completed_predictions": len(rows), "failure_count": len(failures), "predictions_path": str(predictions.path), "predictions_sha256": predictions.sha256, "manifest_path": str(manifest.path), "manifest_sha256": manifest.sha256, "cost_usd_by_model": {key: f"{value:.6f}" for key, value in sorted(costs.items())}, "budget_after_run": controller.ledger.summary()}, indent=2, sort_keys=True))
