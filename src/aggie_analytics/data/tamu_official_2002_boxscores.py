"""Acquire and normalize official SRC-014 2002 box scores from the independently reconstructed BAT-613 index."""
...
CONTRACT_RELATIVE = "configs/tamu_official_2002_boxscore_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2002_boxscore_gate.json"
CONTRACT_ID = "BAT-615-TAMU-OFFICIAL-2002-BOXSCORES-V1"
SOURCE_ID = "SRC-014"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2002_BOXSCORE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2002_BOXSCORES_NORMALIZED"
...
def load_source_index(repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    committed = load_json(repo_root / BAT613_GATE_RELATIVE)
    if committed.get("gate_identity") != PINNED_BAT613_GATE_IDENTITY:
        raise AuthorityViolation("BAT-613 gate identity drifted")
    if committed.get("inventory_identity") != INVENTORY_IDENTITY:
        raise AuthorityViolation("BAT-585 inventory identity rewritten")
    if committed.get("official_index_url") != OFFICIAL_2002_INDEX_URL:
        raise AuthorityViolation("guessed or substituted 2002 official URL")
    if committed.get("payload_identity") != PINNED_BAT613_PAYLOAD_IDENTITY:
        raise AuthorityViolation("BAT-613 payload identity rewritten")
    if committed.get("capture_identity") != PINNED_BAT613_CAPTURE_IDENTITY:
        raise AuthorityViolation("BAT-613 capture identity rewritten")
    if committed.get("box_url_identity") != PINNED_BAT613_BOX_URL_IDENTITY:
    ...
    if (committed.get("capture") or {}).get("raw_sha256") != PINNED_BAT613_RAW_SHA256:
    ...
    if official_2002_index_is_ready(data_root, repo_root):
        reconstructed = reconstruct_official_2002_index(repo_root=repo_root, data_root=data_root)
    ...
    return {
        ...
        "season": SEASON,
        "official_index_url": OFFICIAL_2002_INDEX_URL,
        "box_score_urls": urls,
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "artifact_type": "TAMU_OFFICIAL_2002_BOXSCORES",
        "contract_id": CONTRACT_ID,
        "decision_unit": "POST-TASK-SRC014-2002-OFFICIAL-ACQUISITION-001",
        "jira_key": "BAT-615",
        "source_id": SOURCE_ID,
        ...
        | {
            ...
            "bat_613": "CONSUMED_INDEX_CAPTURE_ONLY",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2002_BOXSCORE_GATE",
        "result": PASS_RESULT if not blocked and counts["normalized_games"] == counts["target_games_total"] else "PARTIAL_OFFICIAL_2002_BOXSCORES",
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("counts", {}).get("ncaa_contest_ids_created"):
        raise AuthorityViolation("NCAA contest IDs fabricated")
    if committed.get("result") not in {PASS_RESULT, "PARTIAL_OFFICIAL_2002_BOXSCORES"}:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation("external 2002 box-score reconstruction was required but the data root is not mounted")
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 2002 box-score gate does not match independent reconstruction")
    payload_path = (
        data_root
