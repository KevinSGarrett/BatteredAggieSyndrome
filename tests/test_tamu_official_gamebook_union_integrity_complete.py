...
class CompactIntegrityCompleteUnionTests(unittest.TestCase):
    ...
    def test_committed_counts_predecessor_and_no_new_games(self) -> None:
        ...
        self.assertEqual(
            self.gate["predecessor_union_identity"], PINNED_BAT607_UNION_IDENTITY
        )
        self.assertEqual(
            self.gate["predecessor_gate_identity"], PINNED_BAT607_GATE_IDENTITY
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.gate["counts"]["official_2004_preserved"], OFFICIAL_2004_EXPECTED
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.gate["upstream_identities"]["bat603_gate_identity"],
            PINNED_BAT603_GATE_IDENTITY,
        )
        self.assertEqual(
        ...
        self.assertTrue(
            all(
                item.get("availability") in {None, "NOT_ESTABLISHED"}
...
class IntegrityCompleteReconstructionAndTamperTests(unittest.TestCase):
    ...
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(
            result["recomputed_upstream"]["bat603_union_manifest_file_sha256"],
            PINNED_BAT603_UNION_MANIFEST_FILE_SHA256,
        )
        self.assertEqual(
            result["recomputed_upstream"]["bat607_union_manifest_file_sha256"],
            PINNED_BAT607_UNION_MANIFEST_FILE_SHA256,
        )
    ...
    def test_missing_predecessor_union_manifests_fail_closed(self) -> None:
        ...
        with _temporarily_moved(bat603_union_manifest_path(DATA_ROOT)):
            with self.assertRaisesRegex(
                AuthorityViolation, "union manifest is missing|data root is not mounted"
            ):
                validate_bat603(
                    repo_root=REPO_ROOT, data_root=DATA_ROOT, require_rebuild=True
                )
    ...
    def test_substituted_and_altered_union_manifests_fail_closed(self) -> None:
        ...
        altered["enriched_official_games"][0]["opponent_candidate"] = (
            "ALTERED WHILE IDENTITY UNCHANGED"
        )
    ...
    def test_missing_successor_union_manifest_fails_closed(self) -> None:
        ...
        with _temporarily_moved(path):
            with self.assertRaisesRegex(
                AuthorityViolation, "union manifest is missing"
            ):
                validate_artifact(
                    repo_root=REPO_ROOT, data_root=DATA_ROOT, require_rebuild=True
                )
    ...
    def test_bat606_row_tampers_fail_against_raw_reconstruction(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=sha
            )
        season = _copy(self.bat606)
        season["games"][0]["source_season"] = 1999
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=season
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=domain
            )
        duplicate = _copy(self.bat606)
        if duplicate["rows"][0]:
            duplicate["rows"][0] = list(duplicate["rows"][0]) + [
                _copy(duplicate["rows"][0][0])
            ]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=duplicate
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=gapped
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=warnings
            )
    ...
    def test_coordinated_bat606_and_outer_rehash_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat606_payload=coordinated
            )
        gate = load_json(REPO_ROOT / GATE_RELATIVE)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
