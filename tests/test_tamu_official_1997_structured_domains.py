...
from aggie_analytics.data.tamu_official_1997_structured_domains import (  # noqa: E402  # pylint: disable=import-error
    AuthorityViolation,
    GATE_RELATIVE,
    compute_identity,
    validate_artifact,
...
class Official1997StructuredDomainsTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            raise unittest.SkipTest("1997 structured-domain gate not materialized")
    ...
    def test_gate_reconstructs_without_writing(self) -> None:
        ...
        self.assertEqual(int(self.gate["counts"]["team_statistics_rows"]), 198)
        self.assertEqual(
            int(self.gate["counts"]["individual_player_statistics_rows"]), 72
        )
        self.assertEqual(int(self.gate["counts"]["drives_rows"]), 0)
        self.assertEqual(int(self.gate["counts"]["play_by_play_rows"]), 0)
