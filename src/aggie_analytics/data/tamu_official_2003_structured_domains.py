...
def load_2003_captures(repo_root: Path, data_root: Path) -> list[dict[str, Any]]:
    ...
    if official_2003_index_is_ready(data_root, repo_root):
        reconstructed_index = reconstruct_official_2003_index(repo_root=repo_root, data_root=data_root)
    ...
    if official_2003_boxscores_are_ready(data_root):
        reconstructed_box = reconstruct_official_2003_boxscores(repo_root=repo_root, data_root=data_root)
        if reconstructed_box["gate"] != bat610:
            raise AuthorityViolation("BAT-610 committed gate does not match independent reconstruction")
    ...
    allowlist = [validate_official_url(str(url)) for url in (bat609.get("box_score_urls") or [])]
    if not allowlist:
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("counts", {}).get("ncaa_contest_ids_created"):
        raise AuthorityViolation("NCAA contest IDs fabricated")
    if committed.get("result") not in {PASS_RESULT, "PARTIAL_OFFICIAL_2003_STRUCTURED_DOMAINS"}:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation("external 2003 structured-domain reconstruction was required but the data root is not mounted")
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 2003 structured-domain gate does not match independent reconstruction")
    payload_path = (
        data_root
