"""Parse source-labeled official 2000 domains from BAT-626 captures (BAT-627)."""
...
from aggie_analytics.data.tamu_official_2000_boxscores import (
    ...
    lake_is_ready as official_2000_boxscores_are_ready,
    reconstruct_objects as reconstruct_official_2000_boxscores,
)
from aggie_analytics.data.tamu_official_2000_season_index import (
    GATE_RELATIVE as BAT625_GATE_RELATIVE,
...
SCHEMA_VERSION = "aggie.data.tamu_official_2000_structured_domains.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2000_structured_domains_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2000_structured_domains_gate.json"
CONTRACT_ID = "BAT-627-TAMU-OFFICIAL-2000-STRUCTURED-DOMAINS-V1"
DECISION_UNIT = "POST-TASK-SRC014-2000-STRUCTURED-DOMAINS-001"
JIRA_KEY = "BAT-627"
SOURCE_ID = "SRC-014"
SEASON = 2000
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2000_STRUCTURED_DOMAIN_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2000_STRUCTURED_DOMAINS_PARSED"
...
MODULE_RELATIVE = "src/aggie_analytics/data/tamu_official_2000_structured_domains.py"
...
PINNED_BAT612_GATE_IDENTITY = (
    "e4c8a1ccc607a9e9f418b30a5cc2dc94ddf4569517b1a16008d8388486f1e0b0"
)
PINNED_BAT612_UNION_IDENTITY = (
    "f6f330d4574cfcb819bd304496e9163e4997d12b0839670b65194bae2a680252"
)
...
PINNED_BAT623_GATE_IDENTITY = (
    "263b576ce1bc67374bd5609b43f7cff54bc3ff72460d0d22d80eb533cad97f6c"
)
...
STRICT_DRIVE_LINE_RE = re.compile(
    r"^[A-Z]{2,5}\s+(?:1st|2nd|3rd|4th|OT)\s+[A-Z]\d{1,2}\s+\d{1,2}:\d{2}\b"
)
EMPTY_DRIVE_CHART_WARNING = "drive_chart_labeled_without_reconstructible_rows"
PINNED_BAT617_GATE_IDENTITY = (
    "b178f7d78645a3c5cc030d1a307a8d2cb896321577b5caa46dc9b51bf5c94551"
)
PINNED_BAT617_PAYLOAD_IDENTITY = (
    "0a2e8e6c510f4d935e8cd6e04f7741491856f46f608e47d62c42f73fab6d4697"
)
...
def reject_nonreconstructible_2000_drives(parsed: dict[str, Any]) -> dict[str, Any]:
    labeled = parsed["domain_coverage"].get("drives") == "PRESENT"
    kept: list[dict[str, Any]] = []
    rejected = 0
    for row in parsed.get("drives") or []:
        text = str(row.get("original_text") or "").strip()
        if STRICT_DRIVE_LINE_RE.match(text):
            kept.append(row)
        else:
            rejected += 1
    ...
    if kept:
        parsed["domain_coverage"]["drives"] = "PRESENT"
    else:
        parsed["domain_coverage"]["drives"] = "ABSENT"
        if labeled or rejected:
            if EMPTY_DRIVE_CHART_WARNING not in warnings:
                warnings.append(EMPTY_DRIVE_CHART_WARNING)
    parsed["warnings"] = warnings
    parsed["rich_structured"] = is_rich_structured(parsed["domain_coverage"])
    return parsed


def load_2000_captures(repo_root: Path, data_root: Path) -> list[dict[str, Any]]:
    ...
    bat626 = load_json(repo_root / BAT626_GATE_RELATIVE)
    if bat626.get("gate_identity") != PINNED_BAT626_GATE_IDENTITY:
        raise AuthorityViolation("BAT-626 2000 acquisition identity rewritten")
    if bat626.get("acquisition_identity") != PINNED_BAT626_ACQUISITION_IDENTITY:
        raise AuthorityViolation("BAT-626 acquisition identity rewritten")
    if bat626.get("dataset_identity") != PINNED_BAT626_DATASET_IDENTITY:
        raise AuthorityViolation("BAT-626 dataset identity rewritten")
    if bat626.get("games_identity") != PINNED_BAT626_GAMES_IDENTITY:
        raise AuthorityViolation("BAT-626 games identity rewritten")
    ...
    if bat625.get("capture_identity") != PINNED_BAT625_CAPTURE_IDENTITY:
        raise AuthorityViolation("BAT-625 capture identity rewritten")
    if bat625.get("box_url_identity") != PINNED_BAT625_BOX_URL_IDENTITY:
        raise AuthorityViolation("BAT-625 box-URL identity rewritten")
    if (bat625.get("capture") or {}).get("raw_sha256") != PINNED_BAT625_RAW_SHA256:
        raise AuthorityViolation("BAT-625 season-index raw hash drifted")
    if official_2000_index_is_ready(data_root, repo_root):
        reconstructed_index = reconstruct_official_2000_index(
            repo_root=repo_root, data_root=data_root
        )
        if reconstructed_index["gate"] != bat625:
            raise AuthorityViolation(
                "BAT-625 committed gate does not match independent reconstruction"
            )
        bat625 = reconstructed_index["gate"]
    if official_2000_boxscores_are_ready(data_root):
        reconstructed_box = reconstruct_official_2000_boxscores(
            repo_root=repo_root, data_root=data_root
        )
        if reconstructed_box["gate"] != bat626:
            raise AuthorityViolation(
                "BAT-626 committed gate does not match independent reconstruction"
            )
    bat623 = load_json(repo_root / BAT623_GATE_RELATIVE)
    if bat623.get("gate_identity") != PINNED_BAT623_GATE_IDENTITY:
        raise AuthorityViolation("BAT-623 2001 structured-domain identity rewritten")
    if bat623.get("payload_identity") != PINNED_BAT623_PAYLOAD_IDENTITY:
        raise AuthorityViolation("BAT-623 payload identity rewritten")
    ...
    allowlist = [
        validate_official_url(str(url)) for url in (bat625.get("box_score_urls") or [])
    ]
    if not allowlist:
        raise AuthorityViolation(
    ...
    if not path.is_file():
        raise AuthorityViolation("BAT-626 capture index missing")
    by_url = {
...
def bind_scoring_summary(
    ...
    for row_order, play in enumerate(plays):
        row = {
            ...
            "upstream_identity": PINNED_BAT626_GATE_IDENTITY,
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    captures = load_2000_captures(repo_root, data_root)
    ...
    for capture in captures:
        ...
        parsed = reject_nonreconstructible_2000_drives(parsed)
    ...
    payload = {
        ...
        "admissions": {
            ...
            "bat_621_payload": "PRESERVED_IMMUTABLE_NOT_REWRITTEN",
            "bat_622_payload": "PRESERVED_IMMUTABLE_NOT_REWRITTEN",
            "bat_623_payload": "PRESERVED_IMMUTABLE_NOT_REWRITTEN",
            "bat_625_payload": "CONSUMED_INDEX_CAPTURE_ONLY",
            "bat_626_payload": "CONSUMED_CAPTURES_ONLY",
            "gap_005": "OPEN",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2000_STRUCTURED_DOMAINS_GATE",
        "result": PASS_RESULT,
...
def validate_compact_gate(committed: Mapping[str, Any], repo_root: Path) -> None:
    ...
    if committed.get("result") not in {
        PASS_RESULT,
        "PARTIAL_OFFICIAL_2000_STRUCTURED_DOMAINS",
    }:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2000 structured-domain reconstruction was required but the data root is not mounted"
        )
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2000 structured-domain gate does not match independent reconstruction"
        )
    payload_path = (
        data_root
