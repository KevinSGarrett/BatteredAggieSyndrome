...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Independently validate official 1999 box-score acquisition."
    )
    ...
    if not args.validate_only:
        ...
        mutations.append(expect_rejection("selected_season_tamper", lambda: _validate(_mutated_gate(gate, selected_seasons=[2000]))))
        ...
        mutations.append(expect_rejection("stale_code_identity", lambda: _validate(_mutated_gate(gate, validator_code_identity="0" * 64))))
        mutations.append(expect_rejection("acquisition_hash_tamper", lambda: _validate(_mutated_gate(gate, acquisition_identity="0" * 64))))
