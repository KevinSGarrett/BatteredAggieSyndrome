...
EXPECTED_HTML_ROWS_IDENTITY = (
    "579f89802cb1ccb5d10b40728d378baed787ce5699243b23d5d63f4859521f98"
)
...
class Compact2006StructuredDomainGateTests(unittest.TestCase):
    ...
    def test_committed_counts_and_upstream_identities(self) -> None:
        ...
        self.assertEqual(
            self.gate["counts"]["individual_player_statistics_present_games"], 13
        )
        ...
        self.assertEqual(
            self.gate["html_table_audit"]["rows_identity"], EXPECTED_HTML_ROWS_IDENTITY
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat595_gate_identity"],
            PINNED_BAT595_GATE_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat595_acquisition_identity"],
            PINNED_BAT595_ACQUISITION_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat595_dataset_identity"],
            PINNED_BAT595_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat591_gate_identity"],
            PINNED_BAT591_GATE_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat591_payload_identity"],
            PINNED_BAT591_PAYLOAD_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat589_gate_identity"],
            PINNED_BAT589_GATE_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat586_gate_identity"],
            PINNED_BAT586_GATE_IDENTITY,
        )
    ...
    def test_html_present_without_rows_fails(self) -> None:
        ...
        with self.assertRaisesRegex(AuthorityViolation, "HTML-table PRESENT"):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
                gate=_mutated(self.gate, html_table_audit=html),
                require_rebuild=False,
            )
