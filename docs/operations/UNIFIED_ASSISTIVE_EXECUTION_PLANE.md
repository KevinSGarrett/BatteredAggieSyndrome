...
- Cursor authentication, repository catalog, and exact `gpt-5.3-codex` catalog presence are verified without exposing the token. The separate USD 200 cap and USD 20 initial stage authorized two serial safety pilots. Both executed serially on exact main and were accepted only after Codex review edits: four total runs used 1,364,409 tokens and settled USD 0.65240990. Pilot 2 tightened exact route-identity admission and exposed a controller finalization-settlement gap that was corrected before the governed follow-up. The remaining real-unit campaign, concurrency gate, utilization evidence, and sustained operation are pending, so Cursor is not operational.
