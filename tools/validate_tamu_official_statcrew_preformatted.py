...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the StatCrew preformatted-domain payload.")
    ...
    if not args.validate_only:
        ...
        if gate["upstream_identities"]["bat586_dataset_identity"] != PRE2010_DATASET_IDENTITY:
            raise AssertionError("committed gate lost the preserved BAT-586 dataset identity")
        if gate["upstream_identities"]["bat589_dataset_identity"] != BOX_2007_DATASET_IDENTITY:
            raise AssertionError("committed gate lost the preserved BAT-589 dataset identity")
