"""Acquire and normalize official SRC-014 2005 box scores from the BAT-599 captured index."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_2005_boxscores.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2005_boxscore_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2005_boxscore_gate.json"
CONTRACT_ID = "BAT-600-TAMU-OFFICIAL-2005-BOXSCORES-V1"
SOURCE_ID = "SRC-014"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2005_BOXSCORE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2005_BOXSCORES_NORMALIZED"
...
OFFICIAL_2005_INDEX_URL = "https://files.12thman.com/history/football/years/2005.html"
SEASON = 2005
...
def load_source_index(repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    gate = load_json(repo_root / BAT599_GATE_RELATIVE)
    if gate.get("gate_identity") != PINNED_BAT599_GATE_IDENTITY:
        raise AuthorityViolation("BAT-599 gate identity drifted")
    if gate.get("inventory_identity") != INVENTORY_IDENTITY:
        raise AuthorityViolation("BAT-585 inventory identity rewritten")
    if gate.get("official_index_url") != OFFICIAL_2005_INDEX_URL:
    ...
    if len(urls) != 11:
        raise AuthorityViolation(f"BAT-599 allowlist is not the 11 official 2005 box URLs: {len(urls)}")
    capture = gate.get("capture") or {}
    return {
        "gate": gate,
        "inventory_gate": inventory_gate,
        "season": SEASON,
        "official_index_url": OFFICIAL_2005_INDEX_URL,
        "box_score_urls": urls,
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "artifact_type": "TAMU_OFFICIAL_2005_BOXSCORES",
        "contract_id": CONTRACT_ID,
        "decision_unit": "POST-TASK-SRC014-2005-OFFICIAL-ACQUISITION-001",
        "jira_key": "BAT-600",
        "source_id": SOURCE_ID,
        ...
        "conflicts": conflicts,
        "admissions": expected_admissions() | {"inventory_identity": INVENTORY_IDENTITY, "gap_005": "OPEN", "bat_523": "IN_PROGRESS", "union_admission": "NOT_ADMITTED", "bat_599": "CONSUMED_INDEX_CAPTURE_ONLY"},
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2005_BOXSCORE_GATE",
        "result": PASS_RESULT if not blocked and counts["normalized_games"] == counts["target_games_total"] else "PARTIAL_OFFICIAL_2005_BOXSCORES",
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("counts", {}).get("ncaa_contest_ids_created"):
        raise AuthorityViolation("NCAA contest IDs fabricated")
    if committed.get("result") not in {PASS_RESULT, "PARTIAL_OFFICIAL_2005_BOXSCORES"}:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation("external 2005 box-score reconstruction was required but the data root is not mounted")
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 2005 box-score gate does not match independent reconstruction")
    payload_path = (
        data_root
