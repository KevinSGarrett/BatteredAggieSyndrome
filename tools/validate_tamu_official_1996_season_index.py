...
def expect_rejection(name: str, operation: Callable[[], Any]) -> dict[str, Any]:
    try:
        ...
        return {"exception": type(exc).__name__, "message": str(exc)[:240], "name": name, "result": "PASS_FAIL_CLOSED"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the official 1996 season-index capture.")
    ...
    if not args.validate_only:
        ...
        mutations.append(
            expect_rejection(
                "invented_season_url",
                lambda: _validate(_mutated_gate(gate, official_index_url="https://files.12thman.com/history/football/years/2004.html")),
            )
        )
        mutations.append(expect_rejection("history_href_proof_removed", lambda: _validate(_mutated_gate(gate, history_href_proof=""))))
