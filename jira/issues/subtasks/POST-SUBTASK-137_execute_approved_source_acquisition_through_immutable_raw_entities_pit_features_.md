...
```json
{
  ...
  "ai_context_notes": [
    ...
    "Read the exact source sections in `jira/sources/issue_source_manifests/POST-SUBTASK-137.json`; inspect only the listed implementation files and prerequisite outputs.",
  ...
  "allowed_modification_paths": [
    ...
    "artifacts/jira_evidence/POST-SUBTASK-137.json"
  ],
  "blocked_reason": "AUTHORITATIVE_TARGET_HOST_AND_PRODUCTION_SOURCE_ACCESS_REQUIRED",
  "blocks": [
    "POST-SUBTASK-138"
  ],
  "canonical_record": "jira/records/issues/subtasks/POST-SUBTASK-137_execute_approved_source_acquisition_through_immutable_raw_entities_pit_features_.json",
  "canonical_source_role": "AUTHORITATIVE_LOCAL_SPECIFICATION",
  "completion_evidence_contract": {
    "acceptance_matrix_required": true,
    "artifact_hashes_required": true,
    "completion_claim_limit": "EMPIRICALLY_VALIDATED",
    "downstream_consumer": "POST-SUBTASK-138",
  ...
  "gap_ids": [],
  "generated_markdown": "jira/issues/subtasks/POST-SUBTASK-137_execute_approved_source_acquisition_through_immutable_raw_entities_pit_features_.md",
  ...
  "operational_jira": {
    "assignee": "",
    "jira_issue_id": "24627",
    "jira_updated_at": "2026-08-09T23:24:13.481-0500",
  ...
  "related_to": [],
  "required_evidence": [
    "`artifacts/release/release_candidate_e2e.json` plus SHA-256/content identity, producer command/version, prerequisite artifact identities, creation time, and validation disposition.",
  ...
  "required_tests": [
    {
      ...
      "expectation": "Run as a regression check after completing POST-SUBTASK-137; retain command, exit code, and relevant output.",
    ...
    },
    {
      ...
      "path": "artifacts/release/release_candidate_e2e.json",
  ...
  "work_packet_path": "jira/ai/work_packets/POST-SUBTASK-137.md",
...
- **EXISTING_AUTOMATED_TEST** / `REGRESSION` — `tests/test_w24_readiness.py` — Run as a regression check after completing POST-SUBTASK-137; retain command, exit code, and relevant output.
- **EXISTING_AUTOMATED_TEST** / `REGRESSION` — `tests/test_w25_final_handoff.py` — Run as a regression check after completing POST-SUBTASK-137; retain command, exit code, and relevant output.
- **EXISTING_AUTOMATED_TEST** / `REGRESSION` — `tools/validate_w24_readiness.py` — Run as a regression check after completing POST-SUBTASK-137; retain command, exit code, and relevant output.
- **CHRONOLOGICAL_REPLAY** / `CHRONOLOGICAL_REPLAY` — `artifacts/release/release_candidate_e2e.json` — Demonstrate cutoff eligibility and negative leakage behavior using pinned chronology; future/same-game/postgame contamination must fail closed.
- **SCIENTIFIC** / `SCIENTIFIC` — `artifacts/release/release_candidate_e2e.json` — Evaluate the precommitted hypothesis/metric against pinned data and splits; preserve null, negative, unstable, and failed results.
- **OPERATIONS** / `OPERATIONS` — `artifacts/release/release_candidate_e2e.json` — Exercise the operating/failure/rollback or recovery path and retain timestamps, identifiers, alerts, and operator decisions.
- **END_TO_END** / `END_TO_END` — `artifacts/release/release_candidate_e2e.json` — Evaluate the complete Story contract from prerequisite evidence through downstream-consumable gate output; unresolved blockers remain blocking.
