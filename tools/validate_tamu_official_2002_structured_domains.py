...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate official 2002 structured domains.")
    ...
    if not args.validate_only:
        ...
        mutations.append(expect_rejection("bat615_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream))))
        upstream = json.loads(json.dumps(gate["upstream_identities"]))
        upstream["bat611_gate_identity"] = "0" * 64
        mutations.append(expect_rejection("bat611_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream))))
