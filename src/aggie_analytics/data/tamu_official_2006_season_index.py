"""Official SRC-014 2006 Texas A&M season-index capture and box-URL discovery (BAT-594)."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_2006_season_index.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2006_season_index_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2006_season_index_gate.json"
EVIDENCE_RELATIVE = "artifacts/jira_evidence/POST-TASK-SRC014-2006-OFFICIAL-INDEX-001.json"
CONTRACT_ID = "BAT-594-TAMU-OFFICIAL-2006-SEASON-INDEX-V1"
DECISION_UNIT = "POST-TASK-SRC014-2006-OFFICIAL-INDEX-001"
JIRA_KEY = "BAT-594"
SOURCE_ID = "SRC-014"
SEASON = 2006
...
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2006_SEASON_INDEX_CAPTURE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2006_SEASON_INDEX_CAPTURED_BOX_URLS_DISCOVERED"
PROTECTED_LANE = "RETAIN_PROTECTED_LANE_BLOCKED"
PAYLOAD_ROOT = "features/tamu_official_2006_season_index/sha256"
HEADER_ALIASES = {
    "date": "date",
    "opponent": "opponent",
    "location": "location",
...
def discover_official_2006_url(data_root: Path) -> dict[str, Any]:
    ...
    if digest != PINNED_HISTORY_INDEX_SHA256:
        raise AuthorityViolation("official history index hash changed; refusing guessed 2006 URL")
    ...
    if match is None:
        raise AuthorityViolation("official history index did not emit a 2006 Results link")
    url = validate_official_url(str(match["official_index_url"]))
    years = {int(item) for item in YEAR_RE.findall(urlsplit(url).path)}
    if years != {SEASON}:
        raise AuthorityViolation(f"official 2006 URL path years {years} are not exactly {{{SEASON}}}")
    if not match.get("url_directly_emitted_by_official_page"):
        raise AuthorityViolation("2006 URL was not directly emitted by the official history index")
    return {"official_index_url": url, "discovery": match, "history_index_sha256": digest}


def assert_official_2006_url(url: str, expected_url: str) -> str:
    validated = validate_official_url(url)
    if validated != expected_url:
        raise AuthorityViolation(f"refusing non-official or guessed 2006 URL: {validated}")
...
def parse_season_game_rows(*, body: bytes, page_url: str, raw_sha256: str) -> dict[str, Any]:
    ...
    if schedule_rows is None:
        raise AuthorityViolation("official 2006 season index emitted no Date/Opponent/Box Score table")
    admitted: list[str] = []
    seen: set[str] = set()
    ...
    for order, raw_row in enumerate(schedule_rows, start=1):
        ...
        if source_date == "" or source_opponent == "":
            raise AuthorityViolation(
                f"2006 game row {order} has an unresolved date or opponent; refusing forced match"
            )
        game_rows.append(
            {
    ...
    if not game_rows:
        raise AuthorityViolation("official 2006 season index emitted no game rows")
...
def fetch_official_2006_season_index(data_root: Path) -> dict[str, Any]:
    ...
    if int(record["status"]) != 200:
        raise AuthorityViolation(f"official 2006 season index HTTP {record['status']}")
    ...
    if disposition != "VERIFIED_OFFICIAL_SCHOOL_PAGE":
        raise AuthorityViolation(f"official 2006 season index not verified: {disposition}")
    return {
        **record,
        "body": body,
...
def build_objects(*, body: bytes, capture: Mapping[str, Any], repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    if int(capture.get("source_season") or 0) != SEASON:
        raise AuthorityViolation("2006 capture season is not 2006")
    if capture.get("temporal_authority") != "UNKNOWN_RETRIEVAL_TIME_ONLY":
        raise AuthorityViolation("2006 capture must remain retrieval-time only")
    ...
    if capture.get("parser_disposition") != "VERIFIED_OFFICIAL_SCHOOL_PAGE":
        raise AuthorityViolation("2006 capture is not a verified official school page")
    raw_sha256 = str(capture.get("raw_sha256") or "")
    if not raw_sha256 or len(raw_sha256) != 64:
        raise AuthorityViolation("2006 capture is missing raw SHA-256")
    ...
    if not parsed["box_score_urls"]:
        raise AuthorityViolation("official 2006 season index emitted no box-score URLs")
    for box_url in parsed["box_score_urls"]:
        validate_official_url(box_url)
        if "years/2006.html" in box_url or box_url.rstrip("/").endswith("/2006.html"):
    ...
    payload = {
        ...
        "source_id": SOURCE_ID,
        "artifact_type": "TAMU_OFFICIAL_2006_SEASON_INDEX_CAPTURE",
    }
    ...
    gate = {
        "admissions": expected_admissions(),
        "artifact_type": "TAMU_OFFICIAL_2006_SEASON_INDEX_GATE",
        "authority": expected_authority(),
        "box_score_urls": parsed["box_score_urls"],
        "box_url_identity": box_url_identity,
        "capture": {
    ...
    if contract.get("jira_key") != JIRA_KEY:
        raise AuthorityViolation("contract Jira key is not BAT-594")
...
def materialize(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    fetched = fetch_official_2006_season_index(data_root)
...
def reconstruct(*, repo_root: Path, data_root: Path, gate: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not raw_rel:
        raise AuthorityViolation("committed 2006 gate is missing raw_relative_path")
    raw_path = data_root / raw_rel
    if not raw_path.is_file():
        raise AuthorityViolation(f"2006 official season index capture is missing: {raw_rel}")
    digest = sha256_file(raw_path)
    expected = (committed.get("capture") or {}).get("raw_sha256")
    if digest != expected:
        raise AuthorityViolation("2006 official season index hash mismatch")
    capture = {
...
def validate_artifact(
    *,
    ...
    if (committed.get("counts") or {}).get("games_admitted_to_union") != 0:
        raise AuthorityViolation("2006 games admitted to a union")
    ...
    if require_rebuild:
        ...
        if rebuilt["gate"]["box_score_urls"] != committed.get("box_score_urls"):
            raise AuthorityViolation("box URLs are not the official 2006 page discovery set")
