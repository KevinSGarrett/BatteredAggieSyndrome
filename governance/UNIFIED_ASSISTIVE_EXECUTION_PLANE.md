...
- Cursor has a separate nontransferable USD 200 hard limit, initially releasing USD 20 for two serial safety pilots. It uses exact live model `gpt-5.3-codex`, reasoning low/medium, `fast=false`, isolated branches, `workOnCurrentBranch=false`, and `autoCreatePR=false`; it currently has zero qualifying real agents.
