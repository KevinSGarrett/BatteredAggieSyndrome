...
from aggie_analytics.data.tamu_official_2003_structured_domains import (  # noqa: E402
    AuthorityViolation,
    GATE_RELATIVE,
...
PREDECESSOR_EXPECTED_GATE_IDENTITY = "758ca462a05f9d67ff5017417626eae666902f054037c76207231287cb3f20e9"
PREDECESSOR_EXPECTED_PAYLOAD_IDENTITY = "8322e53f3ae4b14f7f85b57e30d32664a07b0d5051d4295af681e71083664bf8"
...
class Official2003StructuredParserUnitTests(unittest.TestCase):
...
class Compact2003StructuredDomainGateTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2003 structured-domain gate not materialized yet")
...
@unittest.skipUnless(LAKE_READY, "external BAT-610 captures are not mounted")
class Official2003StructuredDomainReconstructionTests(unittest.TestCase):
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(gate["counts"]["individual_player_statistics_serialized_rows"], 340)
        self.assertEqual(gate["counts"]["drives_serialized_rows"], 349)
        self.assertEqual(gate["counts"]["play_by_play_serialized_rows"], 4515)
        self.assertEqual(gate["counts"]["serialized_rows_total"], 5528)
        ...
        self.assertEqual(gate["upstream_identities"]["bat610_gate_identity"], PINNED_BAT610_GATE_IDENTITY)
