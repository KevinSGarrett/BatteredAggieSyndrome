...
MODULE_RELATIVE = (
    "src/aggie_analytics/data/tamu_official_gamebook_union_2003_expanded.py"
)
...
PINNED_UNION_MANIFEST_FILE_SHA256 = (
    "e827134ecca7308109910fb04676b09997e2e14bdcc8875b168edc5d8b5281a2"
)
PINNED_VALIDATOR_CODE_IDENTITY = (
    "cc32667930047871cdf020c6a0fd4994a70909b5b3cc00b6d571952e99b77dd3"
)
UNION_MANIFEST_NAME = "union_manifest.json"
PINNED_BAT608_UNION_MANIFEST_FILE_SHA256 = (
    "a9347a929cf5c033b8de21d3451c13b1d33b1b3f54c3f3d7286c2129a02315d8"
)
...
def require_authoritative_union_manifest(
    *,
    ...
    if stored != expected_payload:
        raise AuthorityViolation(
            "external 2003-expanded union payload does not match reconstruction"
        )
    serialized = json.dumps(expected_payload, indent=2, sort_keys=True) + "\n"
    if path.read_text(encoding="utf-8-sig") != serialized:
        raise AuthorityViolation(
            "external 2003-expanded union payload serialization does not match reconstruction"
        )
...
def overlay_2003(
    ...
    *,
    ...
    if str(domains.get("source_sha256") or "") != str(row.get("source_sha256") or ""):
        raise AuthorityViolation(
            f"BAT-611 raw hash does not match admitted 2003 game {row.get('url')}"
        )
    if str(domains.get("url") or "") != str(row.get("url") or ""):
        raise AuthorityViolation(
            f"BAT-611 URL does not match admitted 2003 game {row.get('url')}"
        )
...
def validate_bat610_external_payload(
    *,
    ...
    if recomputed != declared:
        raise AuthorityViolation(
            "BAT-610 declared identities do not match recomputed payload content"
        )
    ...
    for key, value in recomputed.items():
        if committed.get(key) != value:
            raise AuthorityViolation(
                f"recomputed BAT-610 {key} does not match the committed gate"
            )
        if (
            value
            != {
            ...
            raise AuthorityViolation(
                f"recomputed BAT-610 {key} does not match the pinned identity"
            )
    if allowed_urls is None:
        raise AuthorityViolation(
            "BAT-609 official 2003 box URLs were not independently reconstructed"
        )
    ...
    if {str(item.get("url") or "") for item in games} != allowed_set:
        raise AuthorityViolation(
            "BAT-610 games are not exactly the BAT-609 official index URLs"
        )
    if set(captures) != allowed_set:
        raise AuthorityViolation(
            "BAT-610 capture membership is not exactly the BAT-609 official index URLs"
        )
    ...
    for item in games:
        ...
        if status not in ADMITTED_STATUSES:
            raise AuthorityViolation(
                f"2003 game lacks official index+URL+SHA admission: {url}"
            )
    ...
    if reconstructed_identities != recomputed:
        raise AuthorityViolation(
            "BAT-610 payload identities do not match independent raw reconstruction"
        )
    return {
...
def validate_bat611_external_payload(
    *,
    ...
    if recomputed != declared:
        raise AuthorityViolation(
            "BAT-611 declared payload identity does not match recomputed payload content"
        )
    ...
    if (
        ...
        or recomputed != PINNED_BAT611_PAYLOAD_IDENTITY
    ):
        raise AuthorityViolation(
            "recomputed BAT-611 payload identity does not match the pinned identity"
        )
    ...
    if (
        len(external_games) != OFFICIAL_2003_EXPECTED
        or len(row_groups) != OFFICIAL_2003_EXPECTED
    ):
    ...
    if set(validated) != set(compact_by_url):
        raise AuthorityViolation(
            "BAT-611 external payload URLs do not match the compact gate"
        )
    return {
        "payload": dict(payload),
...
def reconstruct_objects(
    *,
    ...
    bat611 = validate_bat611_external_payload(
    ...
    if scoring != PRIOR_SCORING + sum(
        1 for item in admitted_2003 if scoring_summary_present(item)
    ):
    ...
    if any(
        ...
        raise AuthorityViolation(
            "2003 overlay is not bound to the independently recomputed BAT-611 payload identity"
        )
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != PRIOR_UNION_CAPTURED_GAMES + OFFICIAL_2003_EXPECTED
    ):
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2003-expanded reconstruction was required but the data root is not mounted"
        )
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2003-expanded union gate does not match independent reconstruction"
        )
