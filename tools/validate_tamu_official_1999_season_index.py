...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Independently validate the official 1999 season-index capture."
    )
    parser.add_argument(
    ...
    if not args.validate_only:
        ...
        mutated_counts = json.loads(json.dumps(gate["counts"]))
        mutated_counts["ncaa_contest_ids_created"] = 1
        mutations.append(
            expect_rejection(
                "ncaa_contest_ids_created",
                lambda: _validate(_mutated_gate(gate, counts=mutated_counts)),
            )
        )
        mutations.append(
            expect_rejection(
