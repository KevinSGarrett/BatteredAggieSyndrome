...
CONTRACT_ID = "BAT-649-TAMU-OFFICIAL-1997-SEASON-INDEX-V1"
...
def build_objects(
    *,
    ...
    resolved_discovery = dict(discovered) if discovered is not None else discover_official_1997_url(data_root)
...
def materialize(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    if _reconstruct_from_lake_only():
        ...
        if sha256_file(raw_path) != str((committed.get("capture") or {}).get("raw_sha256") or ""):
            raise AuthorityViolation("1997 raw capture hash drifted")
