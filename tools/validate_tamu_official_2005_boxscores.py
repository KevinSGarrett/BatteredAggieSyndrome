...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate official 2005 box-score acquisition.")
    ...
    if not args.validate_only:
        ...
        mutations.append(expect_rejection("bat595_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream))))
        upstream2 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream2["bat599_gate_identity"] = "0" * 64
        mutations.append(expect_rejection("bat599_rewrite", lambda: _validate(_mutated_gate(gate, upstream_identities=upstream2))))
        invented = json.loads(json.dumps(gate))
        invented["selected_seasons"] = [2004]
        ...
        counts4["normalized_games"] = 12
        counts4["metadata_only_games"] = 12
