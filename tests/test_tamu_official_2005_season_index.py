...
from aggie_analytics.data.tamu_official_2005_season_index import (  # noqa: E402
    AuthorityViolation,
    GATE_RELATIVE,
...
OFFICIAL_2005_URL = "https://files.12thman.com/history/football/years/2005.html"

FIXTURE_HTML = (
    "<html><body><table id='yearly-stats'>"
    "<tr><th>Date</th><th>Opponent</th><th>Location</th><th>Result</th><th>Box Score</th></tr>"
    "<tr><td>Sep 3</td><td>Clemson</td><td>College Station</td><td>L, 24-25</td>"
    "<td><a href='../stats/2005-2006/ta01-cle.html'>Box Score</a></td></tr>"
    "<tr><td>Sep 17</td><td>SMU</td><td>College Station</td><td>W, 66-8</td>"
    "<td><a href='../stats/2005-2006/ta02-smu.html'>Box Score</a></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
MISSING_BOX_HTML = (
    ...
    "<tr><td>Sep 3</td><td>Clemson</td><td></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
class Official2005UrlAndFixtureTests(unittest.TestCase):
    def test_guessed_year_url_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(AuthorityViolation, "guessed|non-official"):
                build_objects(
                    body=FIXTURE_HTML,
                    capture=_capture("https://files.12thman.com/history/football/years/2006.html", FIXTURE_HTML),
                    repo_root=REPO_ROOT,
                    data_root=Path(tmp),
                )
    ...
    def test_fixture_discovers_source_ordered_rows_and_box_urls(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            stored = persist_capture(
                Path(tmp),
                {
                    **_capture(OFFICIAL_2005_URL, FIXTURE_HTML),
            ...
            self.assertEqual(
                ...
                [
                    "https://files.12thman.com/history/football/stats/2005-2006/ta01-cle.html",
                    "https://files.12thman.com/history/football/stats/2005-2006/ta02-smu.html",
                ],
            )
            ...
            self.assertEqual(
                objects["gate"]["upstream_identities"]["bat594_gate_identity"],
                PINNED_BAT594_GATE_IDENTITY,
            )
            self.assertEqual(objects["gate"]["game_rows"][0]["source_opponent"], "Clemson")
            self.assertEqual(objects["gate"]["game_rows"][0]["source_row_order"], 1)
            self.assertIsNone(objects["gate"]["game_rows"][0]["ncaa_contest_id"])
            self.assertEqual(objects["gate"]["season"], 2005)
            self.assertEqual(objects["gate"]["jira_key"], "BAT-599")

    def test_missing_box_link_is_counted_not_invented(self) -> None:
        parsed = parse_season_game_rows(
            body=MISSING_BOX_HTML,
            page_url=OFFICIAL_2005_URL,
...
class Compact2005GateTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2005 gate not materialized yet")
    ...
    def test_ci_path_validates_without_external_history_index(self) -> None:
        ...
        self.assertEqual(self.gate["official_index_url"], OFFICIAL_2005_URL)
        self.assertEqual(self.gate["upstream_identities"]["bat594_gate_identity"], PINNED_BAT594_GATE_IDENTITY)
...
@unittest.skipUnless(LAKE_READY, "external BAT-599 2005 capture is not mounted")
class Official2005CaptureTests(unittest.TestCase):
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(int(result["scheduled_games"] or 0), 11)
        self.assertEqual(int(result["box_score_url_count"] or 0), 11)
