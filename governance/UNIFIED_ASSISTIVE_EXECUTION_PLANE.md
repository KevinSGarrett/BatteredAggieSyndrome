...
- Cursor has a separate nontransferable USD 200 hard limit, initially releasing USD 20 for qualification and bounded real work. It uses exact live model `gpt-5.3-codex`, reasoning low/medium, `fast=false`, isolated branches, `workOnCurrentBranch=false`, and `autoCreatePR=false`. Four real candidate-only units executed on 2026-08-13 across seven runs, including the two serial pilots and two concurrent nonoverlapping units; all four were accepted, two with Codex review edits, settling a cumulative USD 1.16544630. Six additional real units, broader category coverage, complete weighted utilization evidence, and sustained operation remain pending.
