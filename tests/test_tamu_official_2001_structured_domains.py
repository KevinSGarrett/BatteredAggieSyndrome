...
PREDECESSOR_EXPECTED_CODE_IDENTITY = (
    "a1711813cbf891d333678f089f5b9f516674a753af1805b93ff9e1f87a5d6aa3"
)
...
EXPECTED_CODE_IDENTITY = (
    "3c4d00cc20ec13eb5ba90c82a58993db955994f236cef631c7bd132598eacb71"
)
...
class Official2001StructuredParserUnitTests(unittest.TestCase):
...
class Compact2001StructuredDomainGateTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2001 structured-domain gate not materialized yet")
...
class Official2001StructuredDomainReconstructionTests(unittest.TestCase):
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(
            gate["counts"]["individual_player_statistics_serialized_rows"], 328
        )
        self.assertEqual(gate["counts"]["drives_serialized_rows"], 288)
        self.assertEqual(gate["counts"]["play_by_play_serialized_rows"], 4165)
        self.assertEqual(gate["counts"]["scoring_summary_serialized_rows"], 156)
        self.assertEqual(gate["counts"]["serialized_rows_total"], 5249)
        ...
        msu = next(game for game in gate["games"] if "mfb_415_msu.html" in game["url"])
        osu = next(game for game in gate["games"] if "mfb_417_osu.html" in game["url"])
        oklahoma = next(
            game for game in gate["games"] if "mfb_424_ou.html" in game["url"]
        )
        self.assertEqual(msu["row_counts"]["play_by_play"], 393)
        self.assertEqual(osu["row_counts"]["drives"], 1)
        self.assertEqual(oklahoma["row_counts"]["drives"], 1)
