"""Prove every Cycle #25 material path has exactly one active routing owner."""
...
REGISTRY_PATH = REPO_ROOT / "configs/cycle25_material_ownership_registry.json"
...
class Cycle25RoutingManifestCoverageTest(unittest.TestCase):
    def setUp(self) -> None:
        if not REGISTRY_PATH.is_file():
            self.skipTest("Cycle #25 ownership registry is not in this tree")
        ...
        self.active_owner_is_cycle25 = self.active_owner in self.registry["owners"]

    def _require_active_cycle25_owner(self) -> str:
        if not self.active_owner_is_cycle25:
            self.skipTest(
                f"active routing owner {self.active_owner} is not a Cycle #25 owner"
            )
    ...
    def test_active_manifest_declares_only_paths_it_owns(self) -> None:
        active_owner = self._require_active_cycle25_owner()
        owned = {
            row["path"]
            for row in self.assignments
    ...
    def test_cycle25_authorization_reason_code_is_recorded(self) -> None:
        self._require_active_cycle25_owner()
        self.assertEqual(
            "USER_EXPLICIT_CURSOR_AUTHORIZATION_CYCLE_25",
    ...
    def test_no_cycle25_owner_is_also_a_cycle24_owner(self) -> None:
        cycle24 = _load(REPO_ROOT / "configs/cycle24_material_ownership_registry.json")
        overlap = sorted(set(self.registry["owners"]) & set(cycle24["owners"]))
        self.assertEqual([], overlap)
