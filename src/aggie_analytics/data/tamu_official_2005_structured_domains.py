...
def load_2005_captures(repo_root: Path, data_root: Path) -> list[dict[str, Any]]:
    ...
    allowlist = [
        validate_official_url(str(url)) for url in (bat599.get("box_score_urls") or [])
    ]
    if len(allowlist) != 11:
        raise AuthorityViolation(
            "BAT-599 allowlist is not the 11 official 2005 box URLs"
        )
...
def classify_bound_page(
    data_root: Path, index_relative: str, url: str, source_season: int
) -> dict[str, Any]:
    index_path = data_root / index_relative
    if not index_path.is_file():
        raise AuthorityViolation(
            f"capture index missing for HTML classification: {index_relative}"
        )
    match = next(
        (
            item
            for item in (load_json(index_path).get("captures") or [])
            if item.get("url") == url
        ),
        None,
    )
    if match is None:
        raise AuthorityViolation(
            f"HTML classification URL was not in the bound capture index: {url}"
        )
    ...
    if page["domain_coverage"][PLAY_BY_PLAY] == "PRESENT":
        raise AuthorityViolation(
            "empty or unlabeled HTML tables were promoted to play-by-play"
        )
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    if (
        texas["domain_coverage"][PLAY_BY_PLAY] != "ABSENT"
        or msu["domain_coverage"][PLAY_BY_PLAY] != "ABSENT"
    ):
...
def materialize(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "msu_unknown_tables": objects["gate"]["counts"][
            "montana_state_2007_unknown_tables"
        ],
    }
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("result") not in {
        PASS_RESULT,
        "PARTIAL_OFFICIAL_2005_STRUCTURED_DOMAINS",
    }:
    ...
    for key in ("texas_2006", "montana_state_2007"):
        ...
        if (page.get("domain_coverage") or {}).get("play_by_play") == "PRESENT":
            raise AuthorityViolation(
                "unclassified HTML rows were treated as play-by-play"
            )
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2005 structured-domain reconstruction was required but the data root is not mounted"
        )
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2005 structured-domain gate does not match independent reconstruction"
        )
    payload_path = (
        data_root
