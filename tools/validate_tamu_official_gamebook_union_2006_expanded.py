...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the 2006-expanded official union.")
    ...
    if not args.validate_only:
        ...
        def _validate(tampered: dict[str, Any]) -> Any:
            return validate_artifact(repo_root=repo_root, data_root=data_root, gate=tampered, require_rebuild=False)

        mutations.append(expect_rejection("protected_lane_opened", lambda: _validate(_mutated_gate(gate, protected_lane="OPEN"))))
        mutations.append(expect_rejection("bat592_rewrite", lambda: _validate(_mutated_gate(gate, prior_union_identity="0" * 64))))
        ...
        counts2["new_games_added"] = 12
        mutations.append(expect_rejection("dropped_2006_admission", lambda: _validate(_mutated_gate(gate, counts=counts2))))
        mutations.append(
            expect_rejection(
