...
def specs() -> list[dict[str, Any]]:
    return [
        ...
        make(local_id="POST-SUBTASK-164", import_id=100472, issue_type="Subtask", objective="Pilot quarantine and schema-drift classification with deterministic remediation routing", parent_id="POST-STORY-055", epic_id="POST-EPIC-018", dependencies=["POST-SUBTASK-161"], component="validation-promotion", lane="RESEARCH_LANE", outputs=["artifacts/openai_assist/router_rebalance.json", "artifacts/openai_assist/quarantine_schema_pilot.json", artifact("POST-SUBTASK-164")], touched=["configs/openai_assist_policy.json", "configs/openai_task_registry.json", "src/aggie_analytics/openai_assist", "tools/openai_assist.py", "tools/validate_openai_assist.py", "governance/OPENAI_ASSISTIVE_PLANE.md", "docs/architecture/OPENAI_ASSISTIVE_PLANE.md", "docs/operations/OPENAI_ASSISTIVE_PLANE.md", "artifacts/openai_assist/router_rebalance.json", "artifacts/openai_assist/quarantine_schema_pilot.json", "tests/test_openai_assist.py"], acceptance=["Representative corruption, missingness, schema drift, incompatible mapping, evidence absence, conflict, PIT risk, and target-leakage cases are classified without changing source truth.", "Every remediation route is deterministic-reviewable and affected records/domains remain quarantined until authoritative validators accept them.", "Validated routine classifications begin with GPT-5 Nano Batch; 4o Mini or Luna is used only for a measured task need, complex ambiguity routes to Terra, and only a hard/high-risk residue routes to Sol.", "Meaningful Terra/Sol hard cases and the verified 651bbf...aa523 staged/base/reserve budget are evidenced before completion."], end_to_end="Adopt the revised router, then classify a pinned quarantine/schema-drift sample across Nano and justified higher tiers, preserving candidate-only authority and measuring evidence-verified acceptance per dollar.", maturity="EMPIRICALLY_VALIDATED", labels=["pilot-c", "quarantine"]),
