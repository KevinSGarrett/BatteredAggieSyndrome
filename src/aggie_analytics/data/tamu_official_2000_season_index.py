"""Official SRC-014 2000 Texas A&M season-index capture and box-URL discovery (BAT-625)."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_2000_season_index.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2000_season_index_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2000_season_index_gate.json"
EVIDENCE_RELATIVE = (
    "artifacts/jira_evidence/POST-TASK-SRC014-2000-OFFICIAL-INDEX-001.json"
)
CONTRACT_ID = "BAT-625-TAMU-OFFICIAL-2000-SEASON-INDEX-V1"
DECISION_UNIT = "POST-TASK-SRC014-2000-OFFICIAL-INDEX-001"
JIRA_KEY = "BAT-625"
SOURCE_ID = "SRC-014"
SEASON = 2000
DISCOVERY_PARENT_URL = "https://files.12thman.com/history/football/history/index.html"
OFFICIAL_SEASON_INDEX_URL = "https://files.12thman.com/history/football/years/2000.html"
PINNED_INVENTORY_IDENTITY = (
    "d39d35ff7cfacf2e39a524d0f1fdb97072158c50f84225ed8413771140efaa37"
)
PINNED_INVENTORY_GATE_IDENTITY = (
...
PINNED_BAT588_GATE_IDENTITY = (
    "60fc69c136332de876d511f2020ffbd08282bc4e02256e547d3bcb46222c5ea9"
)
PINNED_BAT588_PAYLOAD_IDENTITY = (
    "a5abf4326d3429ba580d74d3f9d36aaa8267ea0b789af8966ab54f3b55ea792a"
)
PINNED_BAT594_GATE_IDENTITY = (
    "d1f765a73abf0107fcf200562590bfd0212a521df47f9c6b27bb336ad737635c"
)
PINNED_BAT594_PAYLOAD_IDENTITY = (
    "8fd30b3275af348b84c49f90e01fa0e9594c652ecccd5be75154a8e496638b3e"
)
...
MODULE_RELATIVE = "src/aggie_analytics/data/tamu_official_2000_season_index.py"
CODE_BUNDLE_RELATIVE = (MODULE_RELATIVE,)

PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2000_SEASON_INDEX_CAPTURE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2000_SEASON_INDEX_CAPTURED_BOX_URLS_DISCOVERED"
PROTECTED_LANE = "RETAIN_PROTECTED_LANE_BLOCKED"
PAYLOAD_ROOT = "features/tamu_official_2000_season_index/sha256"
HEADER_ALIASES = {
    "date": "date",
    "opponent": "opponent",
    "location": "location",
...
def discover_official_2000_url(data_root: Path) -> dict[str, Any]:
    ...
    if digest != PINNED_HISTORY_INDEX_SHA256:
        raise AuthorityViolation(
            "official history index hash changed; refusing guessed 2000 URL"
        )
    ...
    if match is None:
        raise AuthorityViolation(
            "official history index did not emit a 2000 Results link"
        )
    url = validate_official_url(str(match["official_index_url"]))
    years = {int(item) for item in YEAR_RE.findall(urlsplit(url).path)}
    if years != {SEASON}:
        raise AuthorityViolation(
            f"official 2000 URL path years {years} are not exactly {{{SEASON}}}"
        )
    if not match.get("url_directly_emitted_by_official_page"):
        raise AuthorityViolation(
            "2000 URL was not directly emitted by the official history index"
        )
    return {
...
def assert_official_2000_url(url: str, expected_url: str) -> str:
    validated = validate_official_url(url)
    if validated != expected_url:
        raise AuthorityViolation(
            f"refusing non-official or guessed 2000 URL: {validated}"
        )
...
def parse_season_game_rows(
    *, body: bytes, page_url: str, raw_sha256: str
    ...
    if schedule_rows is None:
        raise AuthorityViolation(
            "official 2000 season index emitted no Date/Opponent/Box Score table"
        )
    admitted: list[str] = []
    seen: set[str] = set()
    ...
    if not game_rows:
        raise AuthorityViolation("official 2000 season index emitted no game rows")
...
def fetch_official_2000_season_index(data_root: Path) -> dict[str, Any]:
    discovered = discover_official_2000_url(data_root)
    url = discovered["official_index_url"]
    record = direct_http_get(url)
    body = record.pop("body")
    if int(record["status"]) != 200:
        raise AuthorityViolation(f"official 2000 season index HTTP {record['status']}")
    ...
    if disposition != "VERIFIED_OFFICIAL_SCHOOL_PAGE":
        raise AuthorityViolation(
            f"official 2000 season index not verified: {disposition}"
        )
    return {
        **record,
        "body": body,
...
def _assert_preserved_2003_index(repo_root: Path) -> None:
    ...
    if (
        bat609.get("official_index_url")
        != "https://files.12thman.com/history/football/years/2003.html"
    ):
...
def _assert_preserved_2006_index(repo_root: Path) -> None:
    ...
    if (
        bat594.get("official_index_url")
        != "https://files.12thman.com/history/football/years/2006.html"
    ):
...
def _assert_preserved_2007_index(repo_root: Path) -> None:
    ...
    if (
        bat588.get("official_index_url")
        != "https://files.12thman.com/history/football/years/2007.html"
    ):
...
def build_objects(
    *, body: bytes, capture: Mapping[str, Any], repo_root: Path, data_root: Path
    ...
    if inventory_gate.get("inventory_identity") != PINNED_INVENTORY_IDENTITY:
        raise AuthorityViolation(
            "BAT-585 inventory identity changed; refusing rewrite or rebound"
        )
    if inventory_gate.get("gate_identity") != PINNED_INVENTORY_GATE_IDENTITY:
        raise AuthorityViolation(
            "BAT-585 inventory gate identity changed; refusing rewrite"
        )
    ...
    if history_path.is_file():
        ...
        if discovered["official_index_url"] != url:
            raise AuthorityViolation(
                "history index no longer emits the pinned 2000 official URL"
            )
    if capture.get("parent_url") != DISCOVERY_PARENT_URL:
        raise AuthorityViolation(
            "2000 capture parent is not the official history index"
        )
    if int(capture.get("source_season") or 0) != SEASON:
        raise AuthorityViolation("2000 capture season is not 2000")
    if capture.get("temporal_authority") != "UNKNOWN_RETRIEVAL_TIME_ONLY":
        raise AuthorityViolation("2000 capture must remain retrieval-time only")
    ...
    if capture.get("parser_disposition") != "VERIFIED_OFFICIAL_SCHOOL_PAGE":
        raise AuthorityViolation("2000 capture is not a verified official school page")
    raw_sha256 = str(capture.get("raw_sha256") or "")
    if not raw_sha256 or len(raw_sha256) != 64:
        raise AuthorityViolation("2000 capture is missing raw SHA-256")
    ...
    if not parsed["box_score_urls"]:
        raise AuthorityViolation("official 2000 season index emitted no box-score URLs")
    for box_url in parsed["box_score_urls"]:
        validate_official_url(box_url)
        if "years/2000.html" in box_url or box_url.rstrip("/").endswith("/2000.html"):
    ...
    payload = {
        ...
        "source_id": SOURCE_ID,
        "artifact_type": "TAMU_OFFICIAL_2000_SEASON_INDEX_CAPTURE",
    ...
    gate = {
        "admissions": expected_admissions(),
        "artifact_type": "TAMU_OFFICIAL_2000_SEASON_INDEX_GATE",
        "authority": expected_authority(),
        "box_score_urls": parsed["box_score_urls"],
        "box_url_identity": box_url_identity,
        "capture": {
    ...
    if contract.get("jira_key") != JIRA_KEY:
        raise AuthorityViolation("contract Jira key is not BAT-625")
...
def materialize(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    fetched = fetch_official_2000_season_index(data_root)
...
def reconstruct(
    *, repo_root: Path, data_root: Path, gate: Mapping[str, Any] | None = None
    ...
    if not raw_rel:
        raise AuthorityViolation("committed 2000 gate is missing raw_relative_path")
    raw_path = data_root / raw_rel
    if not raw_path.is_file():
        raise AuthorityViolation(
            f"2000 official season index capture is missing: {raw_rel}"
        )
    digest = sha256_file(raw_path)
    expected = (committed.get("capture") or {}).get("raw_sha256")
    if digest != expected:
        raise AuthorityViolation("2000 official season index hash mismatch")
    capture = {
...
def validate_artifact(
    *,
    ...
    if (committed.get("counts") or {}).get("games_admitted_to_union") != 0:
        raise AuthorityViolation("2000 games admitted to a union")
    ...
    if len(admitted) != len(set(admitted)):
        raise AuthorityViolation("duplicate admitted box URL")
    if any(
        row.get("ncaa_contest_id") not in {None, ""}
        for row in committed.get("game_rows") or []
    ):
        raise AuthorityViolation("NCAA contest IDs invented")
    if require_rebuild:
        ...
        if rebuilt["gate"]["box_score_urls"] != committed.get("box_score_urls"):
            raise AuthorityViolation(
                "box URLs are not the official 2000 page discovery set"
            )
