...
MODULE_RELATIVE = (
    "src/aggie_analytics/data/tamu_official_gamebook_union_2002_expanded.py"
)
...
PINNED_UNION_IDENTITY = (
    "b8aff3808742d26d4b1fbd9066dd7de412caee5dbc5ee7122879a798953f1be5"
)
PINNED_UNION_MANIFEST_FILE_SHA256 = (
    "3b6f78ba367413d9ccbd16a3ab76921bd9491cc4abf6517caa07a84702fc2dae"
)
PINNED_VALIDATOR_CODE_IDENTITY = (
    "fb875eeb65fde2853106bda0eae2f66d9fb668786832b2ddc2329736b6380f2a"
)
...
def require_authoritative_union_manifest(
    *,
    ...
    if stored != expected_payload:
        raise AuthorityViolation(
            "external 2002-expanded union payload does not match reconstruction"
        )
    serialized = json.dumps(expected_payload, indent=2, sort_keys=True) + "\n"
    if path.read_text(encoding="utf-8-sig") != serialized:
        raise AuthorityViolation(
            "external 2002-expanded union payload serialization does not match reconstruction"
        )
    digest = sha256_file(path)
    if (
        PINNED_UNION_IDENTITY
        and union_identity == PINNED_UNION_IDENTITY
...
def overlay_2002(
    ...
    *,
    ...
    if str(domains.get("source_sha256") or "") != str(row.get("source_sha256") or ""):
        raise AuthorityViolation(
            f"BAT-617 raw hash does not match admitted 2002 game {row.get('url')}"
        )
    if str(domains.get("url") or "") != str(row.get("url") or ""):
        raise AuthorityViolation(
            f"BAT-617 URL does not match admitted 2002 game {row.get('url')}"
        )
...
def validate_bat615_external_payload(
    *,
    ...
    if recomputed != declared:
        raise AuthorityViolation(
            "BAT-615 declared identities do not match recomputed payload content"
        )
    ...
    for key, value in recomputed.items():
        if committed.get(key) != value:
            raise AuthorityViolation(
                f"recomputed BAT-615 {key} does not match the committed gate"
            )
        if (
            value
            != {
            ...
            raise AuthorityViolation(
                f"recomputed BAT-615 {key} does not match the pinned identity"
            )
    if allowed_urls is None:
        raise AuthorityViolation(
            "BAT-613 official 2002 box URLs were not independently reconstructed"
        )
    ...
    if {str(item.get("url") or "") for item in games} != allowed_set:
        raise AuthorityViolation(
            "BAT-615 games are not exactly the BAT-613 official index URLs"
        )
    if set(captures) != allowed_set:
        raise AuthorityViolation(
            "BAT-615 capture membership is not exactly the BAT-613 official index URLs"
        )
    ...
    if reconstructed_identities != recomputed:
        raise AuthorityViolation(
            "BAT-615 payload identities do not match independent raw reconstruction"
        )
    return {
...
def validate_bat617_external_payload(
    *,
    ...
    if recomputed != declared:
        raise AuthorityViolation(
            "BAT-617 declared payload identity does not match recomputed payload content"
        )
    ...
    if (
        ...
        or recomputed != PINNED_BAT617_PAYLOAD_IDENTITY
    ):
        raise AuthorityViolation(
            "recomputed BAT-617 payload identity does not match the pinned identity"
        )
    ...
    if (
        len(external_games) != OFFICIAL_2002_EXPECTED
        or len(row_groups) != OFFICIAL_2002_EXPECTED
    ):
    ...
    if set(validated) != set(compact_by_url):
        raise AuthorityViolation(
            "BAT-617 external payload URLs do not match the compact gate"
        )
    return {
        "payload": dict(payload),
...
def reconstruct_objects(
    *,
    ...
    bat611 = validate_bat617_external_payload(
        repo_root=repo_root, data_root=data_root, payload=bat611_payload
    )
    prior_games = [
    ...
    if OKLAHOMA_2002_UNMATCHED_URL in {
        str(item.get("url") or "") for item in admitted_2002
    }:
    ...
    if (
        len(official_games)
        != PRIOR_ENRICHED_OFFICIAL_GAMES + OFFICIAL_2002_ADMITTED_EXPECTED
    ):
    ...
    if scoring != PRIOR_SCORING + sum(
        1 for item in admitted_2002 if scoring_summary_present(item)
    ):
    ...
    if any(
        item.get("structured_row_payload_identity") != bat611["payload_identity"]
        for item in admitted_2002
    ):
        raise AuthorityViolation(
            "2002 overlay is not bound to the independently recomputed BAT-617 payload identity"
        )
...
def lake_is_ready(data_root: Path) -> bool:
    return (
        ...
        and bool(PINNED_UNION_IDENTITY)
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != PRIOR_UNION_CAPTURED_GAMES + OFFICIAL_2002_ADMITTED_EXPECTED
    ):
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2002-expanded reconstruction was required but the data root is not mounted"
        )
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2002-expanded union gate does not match independent reconstruction"
        )
