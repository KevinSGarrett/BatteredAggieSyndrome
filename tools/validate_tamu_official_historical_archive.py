...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate official Texas A&M historical archive evidence.")
    ...
    if not args.validate_only:
        ...
        mutations.append(expect_rejection("changed_box_count", lambda: _validate(_mutated_gate(gate, counts=counts))))
        urls = json.loads(json.dumps(gate["discovered_box_score_urls"]))
        urls["2010"].append("https://files.12thman.com/history/football/stats/2010-2011/ta99-zzz.html")
        mutations.append(expect_rejection("inserted_box_url", lambda: _validate(_mutated_gate(gate, discovered_box_score_urls=urls))))
        counts = json.loads(json.dumps(gate["counts"]))
        ...
        mutations.append(expect_rejection("forged_completion", lambda: _validate(_mutated_gate(gate, result="FORGED_DONE", classification="PRODUCTION_CHAMPION"))))
