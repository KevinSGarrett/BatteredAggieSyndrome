...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Independently validate the official 2000 season-index capture."
    )
    parser.add_argument(
    ...
    if not args.validate_only:
        ...
        mutations.append(
            expect_rejection(
                ...
                lambda: _validate(
                    _mutated_gate(
                        gate,
                        official_index_url="https://files.12thman.com/history/football/years/2002.html",
                    )
                ),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "changed_raw_sha",
                lambda: _validate(_mutated_gate(gate, capture=capture)),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "inserted_game",
                lambda: _validate(_mutated_gate(gate, game_rows=rows + [extra])),
            )
        )
        mutations.append(
            expect_rejection(
                "removed_game",
                lambda: _validate(_mutated_gate(gate, game_rows=rows[:-1])),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "reordered_games",
                lambda: _validate(_mutated_gate(gate, game_rows=reordered)),
            )
        )
        invented = list(gate["box_score_urls"]) + [
            "https://files.12thman.com/history/football/stats/2000-2001/invented.html"
        ]
        mutations.append(
            expect_rejection(
                "invented_box_url",
                lambda: _validate(_mutated_gate(gate, box_score_urls=invented)),
            )
        )
        reordered_urls = list(reversed(list(gate["box_score_urls"])))
        mutations.append(
            expect_rejection(
                "reordered_box_urls",
                lambda: _validate(_mutated_gate(gate, box_score_urls=reordered_urls)),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "duplicate_admitted_url",
                lambda: _validate(_mutated_gate(gate, box_score_urls=duplicated)),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "bat604_identity_rewrite",
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream)),
            )
        )
        upstream_609 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_609["bat609_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat609_identity_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_609)
                ),
            )
        )
        upstream_613 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_613["bat613_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat613_identity_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_613)
                ),
            )
        )
        upstream_621 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_621["bat621_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat621_identity_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_621)
                ),
            )
        )
        mutations.append(
