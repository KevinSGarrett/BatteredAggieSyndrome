...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the official historical coverage inventory.")
    ...
    if not args.validate_only:
        ...
        invented = json.loads(json.dumps(gate["selected_seasons"]))
        invented[0] = 1894
        mutations.append(expect_rejection("invented_season", lambda: _validate(_mutated_gate(gate, selected_seasons=invented))))
        mutations.append(
            expect_rejection(
                "guessed_url",
                lambda: _validate(
                    _mutated_gate(
                        gate,
                        rejected_seasons=gate["rejected_seasons"]
                        + [{"season": 2007, "reason": "GUESSED", "official_index_url": "https://files.12thman.com/history/football/years/2007.html"}],
                    )
                ),
            )
        )
        mutations.append(expect_rejection("changed_season_ordering", lambda: _validate(_mutated_gate(gate, selected_seasons=list(reversed(gate["selected_seasons"]))))))
