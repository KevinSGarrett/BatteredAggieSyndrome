"""Acquire and normalize official SRC-014 2004 box scores from the BAT-604 captured index."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_2004_boxscores.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2004_boxscore_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2004_boxscore_gate.json"
CONTRACT_ID = "BAT-605-TAMU-OFFICIAL-2004-BOXSCORES-V1"
SOURCE_ID = "SRC-014"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2004_BOXSCORE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2004_BOXSCORES_NORMALIZED"
...
def load_source_index(repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    gate = load_json(repo_root / BAT604_GATE_RELATIVE)
    if gate.get("gate_identity") != PINNED_BAT604_GATE_IDENTITY:
        raise AuthorityViolation("BAT-604 gate identity drifted")
    if gate.get("inventory_identity") != INVENTORY_IDENTITY:
        raise AuthorityViolation("BAT-585 inventory identity rewritten")
    if gate.get("official_index_url") != OFFICIAL_2004_INDEX_URL:
        raise AuthorityViolation("guessed or substituted 2004 official URL")
    if gate.get("payload_identity") != PINNED_BAT604_PAYLOAD_IDENTITY:
        raise AuthorityViolation("BAT-604 payload identity rewritten")
    if (gate.get("capture") or {}).get("raw_sha256") != "db64496a98fb15ff3bed222901269c5901ba78443db4b7a581865157c5e4f17c":
        raise AuthorityViolation("BAT-604 season-index raw hash drifted")
    ...
    if len(urls) != 12:
        raise AuthorityViolation(f"BAT-604 allowlist is not the 12 official 2004 box URLs: {len(urls)}")
    capture = gate.get("capture") or {}
    return {
        "gate": gate,
        "inventory_gate": inventory_gate,
        "season": SEASON,
        "official_index_url": OFFICIAL_2004_INDEX_URL,
        "box_score_urls": urls,
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "artifact_type": "TAMU_OFFICIAL_2004_BOXSCORES",
        "contract_id": CONTRACT_ID,
        "decision_unit": "POST-TASK-SRC014-2004-OFFICIAL-ACQUISITION-001",
        "jira_key": "BAT-605",
        "source_id": SOURCE_ID,
        ...
        | {
            ...
            "bat_604": "CONSUMED_INDEX_CAPTURE_ONLY",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2004_BOXSCORE_GATE",
        "result": PASS_RESULT if not blocked and counts["normalized_games"] == counts["target_games_total"] else "PARTIAL_OFFICIAL_2004_BOXSCORES",
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("counts", {}).get("ncaa_contest_ids_created"):
        raise AuthorityViolation("NCAA contest IDs fabricated")
    if committed.get("result") not in {PASS_RESULT, "PARTIAL_OFFICIAL_2004_BOXSCORES"}:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation("external 2004 box-score reconstruction was required but the data root is not mounted")
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 2004 box-score gate does not match independent reconstruction")
    payload_path = (
        data_root
