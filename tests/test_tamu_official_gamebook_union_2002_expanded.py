...
EXPECTED_UNION_IDENTITY = (
    "a4656c9b4a97f45fd2f501b99114b049cdaa6f1a981f2b94de0f5f81ea8c13e1"
)
...
class CompactExpanded2002UnionTests(unittest.TestCase):
    ...
    def test_committed_counts_predecessor_and_2002_admissions(self) -> None:
        ...
        self.assertEqual(
            self.gate["predecessor_union_identity"], PINNED_BAT612_UNION_IDENTITY
        )
        self.assertEqual(
            self.gate["predecessor_gate_identity"], PINNED_BAT612_GATE_IDENTITY
        )
        self.assertEqual(
            self.gate["validation_contract_version"], VALIDATION_CONTRACT_VERSION
        )
        self.assertEqual(
            self.gate["counts"]["new_games_added"], OFFICIAL_2002_ADMITTED_EXPECTED
        )
        self.assertEqual(
            self.gate["counts"]["official_2002_admitted"],
            OFFICIAL_2002_ADMITTED_EXPECTED,
        )
        self.assertEqual(self.gate["counts"]["official_2002_rejected"], 1)
        self.assertEqual(
            self.gate["counts"]["official_2002_target_games"], OFFICIAL_2002_EXPECTED
        )
        self.assertEqual(
            self.gate["counts"]["predecessor_285_union_games_preserved"],
            PRIOR_UNION_CAPTURED_GAMES,
        )
        ...
        self.assertEqual(
            self.gate["upstream_identities"]["bat615_dataset_identity"],
            PINNED_BAT615_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat617_payload_identity"],
            PINNED_BAT617_PAYLOAD_IDENTITY,
        )
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat615_dataset_identity"],
            PINNED_BAT615_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat617_payload_identity"],
            PINNED_BAT617_PAYLOAD_IDENTITY,
        )
        ...
        self.assertTrue(
            all(
                item.get("parent_url") == OFFICIAL_2002_INDEX_URL
                for item in self.gate["admitted_official_2002_games"]
            )
        )
        self.assertTrue(
            all(
    ...
    def test_prior_identities_are_not_rewritten(self) -> None:
        ...
        bat613 = load_json(
            REPO_ROOT / "artifacts/data_lake/tamu_official_2002_season_index_gate.json"
        )
        ...
        self.assertEqual(
            bat615["acquisition_identity"], PINNED_BAT615_ACQUISITION_IDENTITY
        )
    ...
    def test_parent_url_fallback_is_forbidden(self) -> None:
        with self.assertRaisesRegex(AuthorityViolation, "parent_url missing"):
            compact_official_2002(
                {"url": "https://example.invalid/box"}, OFFICIAL_2002_INDEX_URL
            )
...
@unittest.skipUnless(
    LAKE_READY, "external BAT-612/BAT-615/BAT-617 payloads are not mounted"
)
class Expanded2002ReconstructionAndTamperTests(unittest.TestCase):
    ...
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(
            result["recomputed_upstream"]["bat615_dataset_identity"],
            PINNED_BAT615_DATASET_IDENTITY,
        )
        self.assertEqual(
            result["recomputed_upstream"]["bat617_payload_identity"],
            PINNED_BAT617_PAYLOAD_IDENTITY,
        )
    ...
    def test_bat615_identity_recompute_matches_pins(self) -> None:
        ...
        self.assertEqual(
            recomputed["acquisition_identity"], PINNED_BAT615_ACQUISITION_IDENTITY
        )
        ...
        self.assertTrue(
            all(
                game["parent_url"] == OFFICIAL_2002_INDEX_URL
    ...
    def test_bat617_row_counts_recompute_from_serialized_rows(self) -> None:
        validated = validate_bat617_external_payload(
    ...
    def test_bat615_opponent_score_sha_parent_and_membership_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=opponent
            )
        score = _copy(self.bat615)
        score["games"][0]["tamu_points"] = 99
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=score
            )
        sha = _copy(self.bat615)
        sha["games"][0]["source_sha256"] = "0" * 64
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=sha
            )
        missing = _copy(self.bat615)
        missing["games"][0].pop("parent_url", None)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=missing
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=substituted
            )
        captures = _copy(self.bat615)
        captures["captures"] = captures["captures"][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=captures
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat615_payload=name_only
            )
    ...
    def test_bat617_row_and_coverage_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=changed
            )
        removed = _copy(self.bat617)
        removed["rows"][0] = removed["rows"][0][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=removed
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=added
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=present_zero
            )
        parser = _copy(self.bat617)
        parser["games"][0]["parser_identity"] = "forged.parser.v0"
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=parser
            )
        availability = _copy(self.bat617)
        availability["availability_claim"] = True
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat617_payload=availability
            )
        compact = _copy(
            load_json(
                REPO_ROOT
    ...
    def test_coordinated_tamper_plus_recomputed_outer_identity_fails(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
                bat615_payload=bat615,
                bat617_payload=bat617,
            )
