...
The versioned state implementation is `src/aggie_analytics/assistive_plane/controller_state.py`; the fail-closed inventory scheduler is `src/aggie_analytics/assistive_plane/scheduler_runtime.py`; and the independent inspection implementation is `src/aggie_analytics/assistive_plane/watchdog.py`. The scheduler consumes only a clean-current-main promoted inventory pointer, verifies snapshot freshness, validation identity, coverage, authority, and Git binding, and then records durable cycles and idle-with-ready-work intervals. A cycle with zero dispatch never establishes operational scheduling. Provider dispatch, cold-boot behavior, provider campaigns, and the seven-day soak remain required external evidence.
...
This revision safety layer repairs controller continuity but is not the dispatch engine. Granular discovery, leases, admitted adapters, polling, validation, review queue, settlement, cleanup, requeue, reconciliation, and restart recovery remain required before the state machine can support operational completion.
