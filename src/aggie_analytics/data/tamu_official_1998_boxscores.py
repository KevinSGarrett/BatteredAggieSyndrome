"""Acquire and normalize official SRC-014 1998 box scores from BAT-634 allowlist."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_1998_boxscores.v1"
CONTRACT_RELATIVE = "configs/tamu_official_1998_boxscore_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_1998_boxscore_gate.json"
CONTRACT_ID = "BAT-635-TAMU-OFFICIAL-1998-BOXSCORES-V1"
SOURCE_ID = "SRC-014"
SEASON = 1998
OFFICIAL_1998_INDEX_URL = "https://files.12thman.com/history/football/years/1998.html"
CAPTURE_INDEX_RELATIVE = "features/tamu_official_1998_boxscores/capture_index.json"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_1998_BOXSCORE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_1998_BOXSCORES_NORMALIZED"
...
PINNED_BAT634_PAYLOAD_IDENTITY = (
    "25cf06fb9507aaac6f6211e6c59ad4d51e045f944da8c7fb7b530620d8e03b23"
)
...
BAT634_GATE_RELATIVE = "artifacts/data_lake/tamu_official_1998_season_index_gate.json"
...
MODULE_RELATIVE = "src/aggie_analytics/data/tamu_official_1998_boxscores.py"
...
def load_source_index(repo_root: Path) -> dict[str, Any]:
    ...
    gate = load_json(repo_root / BAT634_GATE_RELATIVE)
    if gate.get("gate_identity") != PINNED_BAT634_GATE_IDENTITY:
        raise AuthorityViolation("BAT-634 gate identity drifted")
    if gate.get("payload_identity") != PINNED_BAT634_PAYLOAD_IDENTITY:
        raise AuthorityViolation("BAT-634 payload identity rewritten")
    if gate.get("official_index_url") != OFFICIAL_1998_INDEX_URL:
    ...
    if not urls:
        raise AuthorityViolation("BAT-634 allowlist emitted no official 1998 box URLs")
...
def compact_official_1998_capture(record: Mapping[str, Any], *, source_order: int) -> dict[str, Any]:
    ...
    if not response_sha256 or not raw_sha256 or response_sha256 != raw_sha256:
...
def acquire_missing(
    *,
    ...
    for target in targets:
        ...
        captures.append(compact_official_1998_capture(stored, source_order=source_order))
    return sorted(captures, key=lambda item: int(item["source_order"]))


def index_rows_from_phase6(source: Mapping[str, Any], allowed: frozenset[str]) -> list[dict[str, Any]]:
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    index_rows = index_rows_from_phase6(source, allowed)
    ...
    payload = {
        ...
        "artifact_type": "TAMU_OFFICIAL_1998_BOXSCORES",
        "contract_id": CONTRACT_ID,
        "decision_unit": "POST-TASK-SRC014-1998-OFFICIAL-ACQUISITION-001",
        "jira_key": "BAT-635",
        "source_id": SOURCE_ID,
        ...
        | {
            "inventory_identity": INVENTORY_IDENTITY,
            "bat_634": "CONSUMED_INDEX_CAPTURE_ONLY",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_1998_BOXSCORE_GATE",
        "result": PASS_RESULT if not blocked and counts["normalized_games"] == counts["target_games_total"] else "PARTIAL_OFFICIAL_1998_BOXSCORES",
        ...
        "upstream_identities": {
            ...
            "bat634_payload_identity": PINNED_BAT634_PAYLOAD_IDENTITY,
    ...
    return {"contract": contract, "gate": gate, "payload": payload, "selected_seasons": [SEASON]}
...
def validate_compact_gate(committed: Mapping[str, Any], repo_root: Path) -> None:
    ...
    if upstream.get("bat634_gate_identity") != PINNED_BAT634_GATE_IDENTITY:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation("external 1998 reconstruction was required but data root is not mounted")
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 1998 box-score gate does not match independent reconstruction")
    payload_path = (
