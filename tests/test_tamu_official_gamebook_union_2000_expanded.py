...
EXPECTED_GATE_IDENTITY = "8d8fcbd413524ce65321212c3c68efe481ddc8e6dda73ab0d6824826dd29e3b2"
EXPECTED_UNION_IDENTITY = "de887925b47100d9130873cc2878d3931a88f5d5a2ecf2a6b28c22b12a1d9b35"
...
class CompactExpanded2000UnionTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2000-expanded union gate not materialized yet")
        self.gate = load_json(path)

    def test_committed_counts_predecessor_and_2000_admissions(self) -> None:
        ...
        self.assertEqual(
            self.gate["predecessor_union_identity"], PINNED_BAT624_UNION_IDENTITY
        )
        self.assertEqual(
            self.gate["predecessor_gate_identity"], PINNED_BAT624_GATE_IDENTITY
        )
        self.assertEqual(
            self.gate["validation_contract_version"], VALIDATION_CONTRACT_VERSION
        )
        self.assertEqual(
            self.gate["counts"]["new_games_added"], OFFICIAL_2000_ADMITTED_EXPECTED
        )
        self.assertEqual(
            self.gate["counts"]["official_2000_admitted"],
            OFFICIAL_2000_ADMITTED_EXPECTED,
        )
        self.assertEqual(self.gate["counts"]["official_2000_rejected"], 3)
        self.assertEqual(self.gate["counts"]["official_2001_rejected"], 0)
        self.assertEqual(
            self.gate["counts"]["official_2000_target_games"], OFFICIAL_2000_EXPECTED
        )
        self.assertEqual(self.gate["counts"]["official_2002_rejected"], 1)
        self.assertEqual(
            self.gate["counts"]["predecessor_308_union_games_preserved"],
            PRIOR_UNION_CAPTURED_GAMES,
        )
        self.assertEqual(self.gate["counts"]["union_captured_games"], 317)
        self.assertEqual(self.gate["counts"]["rich_structured_games"], 304)
        self.assertEqual(self.gate["counts"]["metadata_only_games"], 13)
        self.assertEqual(self.gate["counts"]["scoring_summary_present_games"], 114)
        self.assertEqual(self.gate["counts"]["unmatched_rejected"], 4)
        self.assertEqual(self.gate["counts"]["ncaa_contest_ids_created"], 0)
        self.assertEqual(self.gate["counts"]["overlays_became_rich_this_phase"], 9)
        self.assertEqual(
        ...
        self.assertEqual(
            self.gate["upstream_identities"]["bat624_union_identity"],
            PINNED_BAT624_UNION_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat626_dataset_identity"],
            PINNED_BAT626_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat627_payload_identity"],
            PINNED_BAT627_PAYLOAD_IDENTITY,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat626_dataset_identity"],
            PINNED_BAT626_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat627_payload_identity"],
            PINNED_BAT627_PAYLOAD_IDENTITY,
        )
        self.assertEqual(
        ...
        self.assertEqual(len(self.gate["admitted_official_2000_games"]), 9)
        self.assertEqual(len(self.gate["rejected_official_2000_games"]), 3)
        self.assertEqual(len(self.gate["enriched_official_games"]), 114)
        self.assertTrue(
            all(
                item.get("parent_url") == OFFICIAL_2000_INDEX_URL
                for item in self.gate["admitted_official_2000_games"]
            )
        )
        self.assertTrue(
            all(
    ...
    def test_prior_identities_are_not_rewritten(self) -> None:
        ...
        bat626 = load_json(
            REPO_ROOT / "artifacts/data_lake/tamu_official_2000_boxscore_gate.json"
        )
        bat627 = load_json(
            REPO_ROOT
        ...
        bat625 = load_json(
            REPO_ROOT / "artifacts/data_lake/tamu_official_2000_season_index_gate.json"
        )
        ...
        self.assertEqual(bat624["union_identity"], PINNED_BAT624_UNION_IDENTITY)
        self.assertEqual(bat624["gate_identity"], PINNED_BAT624_GATE_IDENTITY)
        ...
        self.assertEqual(bat626["gate_identity"], PINNED_BAT626_GATE_IDENTITY)
        self.assertEqual(bat626["dataset_identity"], PINNED_BAT626_DATASET_IDENTITY)
        self.assertEqual(bat627["payload_identity"], PINNED_BAT627_PAYLOAD_IDENTITY)
        self.assertEqual(bat627["gate_identity"], PINNED_BAT627_GATE_IDENTITY)
        self.assertEqual(bat625["gate_identity"], PINNED_BAT625_GATE_IDENTITY)
        self.assertEqual(
            load_json(
                REPO_ROOT
    ...
    def test_parent_url_fallback_is_forbidden(self) -> None:
        ...
        with self.assertRaisesRegex(AuthorityViolation, "does not match BAT-625"):
            compact_official_2000(
                {
                    "url": "https://example.invalid/box",
                    "parent_url": "https://files.12thman.com/history/football/years/2001.html",
                    "source_season": 2000,
    ...
    def test_ncaa_id_rejected_url_oklahoma_and_429_fail(self) -> None:
        ...
        unmatched = _copy(self.gate["enriched_official_games"])
        unmatched.append(
            {
        ...
        with self.assertRaisesRegex(AuthorityViolation, "unmatched official 2000"):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
                gate=_mutated(self.gate, enriched_official_games=unmatched),
                require_rebuild=False,
            )


@unittest.skipUnless(
    LAKE_READY, "external BAT-624/BAT-626/BAT-627 payloads are not mounted"
)
class Expanded2000ReconstructionAndTamperTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(result["counts"]["new_games_added"], 9)
        self.assertEqual(result["counts"]["union_captured_games"], 317)
        self.assertEqual(
            result["recomputed_upstream"]["bat626_dataset_identity"],
            PINNED_BAT626_DATASET_IDENTITY,
        )
        self.assertEqual(
            result["recomputed_upstream"]["bat627_payload_identity"],
            PINNED_BAT627_PAYLOAD_IDENTITY,
        )
    ...
    def test_bat622_identity_recompute_matches_pins(self) -> None:
        ...
        self.assertEqual(recomputed["dataset_identity"], PINNED_BAT626_DATASET_IDENTITY)
        self.assertEqual(
            recomputed["acquisition_identity"], PINNED_BAT626_ACQUISITION_IDENTITY
        )
        self.assertEqual(recomputed["games_identity"], PINNED_BAT626_GAMES_IDENTITY)
        validated = validate_bat626_external_payload(
            repo_root=REPO_ROOT,
            data_root=DATA_ROOT,
        ...
        self.assertTrue(
            all(
                game["parent_url"] == OFFICIAL_2000_INDEX_URL
    ...
    def test_bat623_row_counts_recompute_from_serialized_rows(self) -> None:
        validated = validate_bat627_external_payload(
            repo_root=REPO_ROOT, data_root=DATA_ROOT
        )
        self.assertEqual(validated["payload_identity"], PINNED_BAT627_PAYLOAD_IDENTITY)
        total_rows = 0
        ...
        self.assertEqual(total_rows, 5479)
    ...
    def test_bat622_opponent_score_sha_parent_and_membership_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=opponent
            )
        score = _copy(self.bat622)
        score["games"][0]["tamu_points"] = 99
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=score
            )
        sha = _copy(self.bat622)
        sha["games"][0]["source_sha256"] = "0" * 64
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=sha
            )
        missing = _copy(self.bat622)
        missing["games"][0].pop("parent_url", None)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=missing
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=substituted
            )
        captures = _copy(self.bat622)
        captures["captures"] = captures["captures"][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=captures
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat626_payload=name_only
            )
    ...
    def test_bat623_row_and_coverage_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=changed
            )
        removed = _copy(self.bat623)
        removed["rows"][0] = removed["rows"][0][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=removed
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=added
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=present_zero
            )
        parser = _copy(self.bat623)
        parser["games"][0]["parser_identity"] = "forged.parser.v0"
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=parser
            )
        availability = _copy(self.bat623)
        availability["availability_claim"] = True
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat627_payload=availability
            )
        compact = _copy(
            load_json(
                REPO_ROOT
    ...
    def test_coordinated_tamper_plus_recomputed_outer_identity_fails(self) -> None:
        ...
        bat623["payload_identity"] = recompute_bat627_payload_identity(bat623)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
                bat626_payload=bat622,
                bat627_payload=bat623,
            )
