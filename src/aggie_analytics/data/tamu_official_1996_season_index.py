"""Official SRC-014 1996 season-index capture and box-URL discovery."""
...
OFFICIAL_SEASON_INDEX_URL = "https://files.12thman.com/history/football/years/1996.html"
...
MODULE_RELATIVE = "src/aggie_analytics/data/tamu_official_1996_season_index.py"
CODE_BUNDLE_RELATIVE = (MODULE_RELATIVE,)
PAYLOAD_ROOT = "features/tamu_official_1996_season_index/sha256"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_1996_SEASON_INDEX_CAPTURE_CANDIDATE_ONLY"
...
def discover_official_1996_url(data_root: Path) -> dict[str, Any]:
    ...
    if digest != PINNED_HISTORY_INDEX_SHA256:
        raise AuthorityViolation("official history index hash changed; refusing guessed 1996 URL")
    text = history_path.read_text(encoding="latin-1", errors="replace")
    if HISTORY_HREF_PROOF not in text:
        raise AuthorityViolation("official history index did not emit ../years/1996.html")
    ...
    if match is None:
        raise AuthorityViolation("official history index did not emit a 1996 Results link")
...
def fetch_official_1996_season_index(data_root: Path) -> dict[str, Any]:
    ...
    if int(record["status"]) != 200:
        raise AuthorityViolation(f"official 1996 season index HTTP {record['status']}")
    ...
    if disposition != "VERIFIED_OFFICIAL_SCHOOL_PAGE":
        raise AuthorityViolation(f"official 1996 season index not verified: {disposition}")
    return {
        **record,
        "body": body,
...
def build_objects(*, body: bytes, capture: Mapping[str, Any], repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    payload = {
        "artifact_type": "TAMU_OFFICIAL_1996_SEASON_INDEX_CAPTURE",
    ...
    gate = {
        "artifact_type": "TAMU_OFFICIAL_1996_SEASON_INDEX_GATE",
        "schema_version": SCHEMA_VERSION,
