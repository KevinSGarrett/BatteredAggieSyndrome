...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the 2007 official gamebook union.")
    ...
    if not args.validate_only:
        ...
        mutations.append(
            expect_rejection(
                "prior_union_rewritten",
                lambda: _validate(_mutated_gate(gate, prior_union_identity="0" * 64)),
            )
        )
        ...
        if gate.get("prior_union_identity") != PRIOR_UNION_IDENTITY:
            raise AssertionError("committed gate lost the preserved BAT-587 226-game union identity")
