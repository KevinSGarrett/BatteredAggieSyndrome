...
def specs() -> list[dict[str, Any]]:
    return [
        ...
        make(local_id="POST-STORY-055", import_id=100466, issue_type="Story", objective="Bounded OpenAI assistive extraction, entity, quarantine, and availability pilots", parent_id="POST-EPIC-018", epic_id="POST-EPIC-018", dependencies=["POST-SUBTASK-161"], component="data-sources", lane="RESEARCH_LANE", outputs=[artifact("POST-STORY-055")], touched=[], acceptance=["Pilots A-C receive empirical gold/reference/cheaper-model comparisons and candidate-only dispositions; Pilot D remains conditional until timestamped evidence exists.", "Unsupported facts, false merges, leakage, and fabricated timestamps/statistics remain zero for accepted candidates."], end_to_end="Compare all bounded pilot evidence, preserve failures and abstentions, and decide per format whether Luna Batch, Terra review, Sol residue, deterministic-only, or rejection is justified.", maturity="EMPIRICALLY_VALIDATED", labels=["aggregate-gate"]),
        make(local_id="POST-STORY-056", import_id=100467, issue_type="Story", objective="Empirical OpenAI scale-out, usage accounting, cleanup, and handoff", parent_id="POST-EPIC-018", epic_id="POST-EPIC-018", dependencies=["POST-SUBTASK-162", "POST-SUBTASK-163", "POST-SUBTASK-164"], component="release-readiness", lane="PROTECTED_GATE", outputs=[artifact("POST-STORY-056")], touched=[], acceptance=["Only empirically validated routine formats scale to Luna Batch, with ambiguity routed to Terra and a small hard residue to Sol.", "Handoff reports exact jobs, models, efforts, tokens, dollars, remaining budget, dispositions, improvements, cleanup, and unresolved reviews."], end_to_end="Prove scale-out remains inside empirical acceptance, candidate authority, budget, storage, cleanup, and protected-nonclaim boundaries.", maturity="EMPIRICALLY_VALIDATED", labels=["aggregate-gate"]),
        ...
        make(local_id="POST-SUBTASK-164", import_id=100472, issue_type="Subtask", objective="Pilot quarantine and schema-drift classification with deterministic remediation routing", parent_id="POST-STORY-055", epic_id="POST-EPIC-018", dependencies=["POST-SUBTASK-161"], component="validation-promotion", lane="RESEARCH_LANE", outputs=["artifacts/openai_assist/quarantine_schema_pilot.json", artifact("POST-SUBTASK-164")], touched=["configs/openai_task_registry.json", "artifacts/openai_assist/quarantine_schema_pilot.json", "tests/test_openai_assist.py"], acceptance=["Representative corruption, missingness, schema drift, incompatible mapping, evidence absence, conflict, PIT risk, and target-leakage cases are classified without changing source truth.", "Every remediation route is deterministic-reviewable and affected records/domains remain quarantined until authoritative validators accept them.", "Routine validated classifications may use Luna Batch; ambiguity escalates only when measured validation failure justifies it."], end_to_end="Classify a pinned quarantine/schema-drift sample, compare deterministic and model routes, and prove outputs only prioritize remediation while authoritative quarantine remains unchanged.", maturity="EMPIRICALLY_VALIDATED", labels=["pilot-c", "quarantine"]),
...
def main() -> int:
    ...
    if any(value in set(range(100464, 100475)) for value in all_import_ids):
