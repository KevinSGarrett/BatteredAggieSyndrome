...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_1996_STRUCTURED_DOMAINS_GATE",
        "result": "PASS_OFFICIAL_1996_STRUCTURED_DOMAINS" if counts["games_with_structured_rows"] > 0 else "PARTIAL_OFFICIAL_1996_STRUCTURED_DOMAINS",
        "classification": "TAMU_SRC014_OFFICIAL_1996_STRUCTURED_DOMAINS_CANDIDATE_ONLY",
...
def validate_artifact(*, repo_root: Path, data_root: Path, gate: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation("committed 1996 structured-domains gate does not match independent reconstruction")
    ...
    if not payload_path.is_file():
        raise AuthorityViolation("external 1996 structured payload missing")
    if load_json(payload_path) != expected["payload"]:
        raise AuthorityViolation("external 1996 structured payload mismatch")
