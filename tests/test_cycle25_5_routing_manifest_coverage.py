...
class Cycle255RoutingManifestCoverageTest(unittest.TestCase):
    ...
    def test_active_manifest_declares_only_assigned_cycle25_5_paths(self) -> None:
        self._require_active_cycle255_owner()
        assigned = {row["path"] for row in self.assignments}
        material = {
            row["path"]
            for row in self.assignments
            if row["kind"] == "MATERIAL" and row["owner"] in self.registry["owners"]
        }
        process_only = {
            row["path"] for row in self.assignments if row["kind"] in PROCESS_ONLY_KINDS
        }
        declared = set(self.manifest["changed_paths"])
        self.assertEqual(sorted(declared), sorted(assigned))
        undeclared = sorted(declared - material - process_only)
