...
class OfficialRowCorpusIntegrityUnitTests(unittest.TestCase):
    ...
    def test_forbidden_urls_include_oklahoma_and_preserved_rejections(self) -> None:
        ...
        self.assertTrue(OFFICIAL_2000_UNMATCHED_URLS <= FORBIDDEN_URLS)
    ...
    def test_code_identity_hashes_enumerated_bundle_and_is_not_a_literal(self) -> None:
        ...
        self.assertNotIn(
            'return "abaad66cbc05c9f98d8388e42e3195164458391b2458d9a79491f8ca0b2636c8"',
            function_source,
        )
        self.assertNotEqual(
            identity, "abaad66cbc05c9f98d8388e42e3195164458391b2458d9a79491f8ca0b2636c8"
        )
    ...
    def test_unauthorized_parser_default_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(
            AuthorityViolation, "upstream parser/default change"
        ):
            bind_corpus_row(
                raw,
                union_game={
    ...
    def test_unmounted_data_root_validates_compact_gate_only(self) -> None:
        ...
        empty = Path(tempfile.mkdtemp(prefix="bat629-empty-"))
        result = validate_artifact(
            repo_root=REPO_ROOT, data_root=empty, require_rebuild=False
        )
        ...
        with self.assertRaisesRegex(AuthorityViolation, "data root is not mounted"):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=empty, require_rebuild=True
            )


@unittest.skipUnless(
    LAKE_READY, "mounted raw upstreams, BAT-628 union, and BAT-620 corpus are required"
)
class OfficialRowCorpusIntegrityMaterialTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._tmp = Path(tempfile.mkdtemp(prefix="bat629-"))
    ...
    def _rewrite_child(self, staged: Path, domain: str, rows: list[dict]) -> None:
        ...
        manifest["child_payloads"][domain]["sha256"] = hashlib.sha256(
            payload
        ).hexdigest()
    ...
    def test_reconstruct_matches_committed_gate_and_preserves_union_membership(
        ...
        self.assertEqual(
            reconstructed["gate"]["gate_identity"], self.gate["gate_identity"]
        )
        self.assertEqual(validated["dataset_identity"], self.dataset_identity)
        self.assertEqual(self.gate["union_identity"], PINNED_BAT628_UNION_IDENTITY)
        self.assertEqual(
            self.gate["predecessor_dataset_identity"], PINNED_BAT620_DATASET_IDENTITY
        )
        self.assertEqual(self.gate["counts"]["games"], 114)
        self.assertGreater(self.gate["counts"]["scoring_summary_serialized_rows"], 0)
        self.assertEqual(self.gate["selected_seasons"], list(SELECTED_SEASONS))
        self.assertFalse(
            self.gate["scientific_nonclaims"]["predecessor_rows_declared_invalid"]
        )
        self.assertNotEqual(
            self.gate["validator_code_identity"],
            "abaad66cbc05c9f98d8388e42e3195164458391b2458d9a79491f8ca0b2636c8",
        )
        consumed = consume_corpus(
            data_root=DATA_ROOT, dataset_identity=self.dataset_identity
        )
        self.assertTrue(consumed["scoring_summary"])
    ...
    def test_changed_code_with_stale_code_identity(self) -> None:
        with mock.patch(
            "aggie_analytics.data.tamu_official_2000_2009_structured_row_corpus.compute_code_identity",
            return_value="0" * 64,
        ):
    ...
    def test_changed_child_payload(self) -> None:
        staged = self._stage()
        lines = (
            (staged / CHILD_FILENAMES["team_statistics"])
    ...
    def test_availability_promotion_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(
            AuthorityViolation, "participation promoted to availability"
        ):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
