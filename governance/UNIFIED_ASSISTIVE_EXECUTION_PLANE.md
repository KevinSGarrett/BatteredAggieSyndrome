...
- Cursor has a separate nontransferable USD 200 hard limit, initially releasing USD 20 for two serial safety pilots. It uses exact live model `gpt-5.3-codex`, reasoning low/medium, `fast=false`, isolated branches, `workOnCurrentBranch=false`, and `autoCreatePR=false`. Both serial pilots executed on 2026-08-13 as real candidate-only agents and produced two accepted-with-review-edits results across four runs, settling a cumulative USD 0.65240990. This satisfies only the serial-pilot dependency; the remaining real-unit campaign, bounded-concurrency gate, utilization evidence, and sustained operation remain pending.
