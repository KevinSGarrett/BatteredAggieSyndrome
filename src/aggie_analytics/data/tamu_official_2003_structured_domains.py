"""Parse source-labeled official 2003 domains from BAT-610 captures (BAT-611)."""
...
from aggie_analytics.data.tamu_official_2003_boxscores import (
    ...
    lake_is_ready as official_2003_boxscores_are_ready,
    reconstruct_objects as reconstruct_official_2003_boxscores,
)
from aggie_analytics.data.tamu_official_2003_season_index import (
    GATE_RELATIVE as BAT609_GATE_RELATIVE,
...
SCHEMA_VERSION = "aggie.data.tamu_official_2003_structured_domains.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2003_structured_domains_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2003_structured_domains_gate.json"
CONTRACT_ID = "BAT-611-TAMU-OFFICIAL-2003-STRUCTURED-DOMAINS-V1"
DECISION_UNIT = "POST-TASK-SRC014-2003-STRUCTURED-DOMAINS-001"
JIRA_KEY = "BAT-611"
SOURCE_ID = "SRC-014"
SEASON = 2003
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2003_STRUCTURED_DOMAIN_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2003_STRUCTURED_DOMAINS_PARSED"
...
PINNED_BAT609_RAW_SHA256 = (
    "b4f48fa72d7658643a3ad44f4cedcdd2e080986b7a7be88bcac3e12c0cdf4e5a"
)
...
def load_2003_captures(repo_root: Path, data_root: Path) -> list[dict[str, Any]]:
    ...
    if bat610.get("acquisition_identity") != PINNED_BAT610_ACQUISITION_IDENTITY:
        raise AuthorityViolation("BAT-610 acquisition identity rewritten")
    if bat610.get("dataset_identity") != PINNED_BAT610_DATASET_IDENTITY:
        raise AuthorityViolation("BAT-610 dataset identity rewritten")
    if bat610.get("games_identity") != PINNED_BAT610_GAMES_IDENTITY:
        raise AuthorityViolation("BAT-610 games identity rewritten")
    ...
    if bat609.get("capture_identity") != PINNED_BAT609_CAPTURE_IDENTITY:
        raise AuthorityViolation("BAT-609 capture identity rewritten")
    if bat609.get("box_url_identity") != PINNED_BAT609_BOX_URL_IDENTITY:
        raise AuthorityViolation("BAT-609 box-URL identity rewritten")
    if (bat609.get("capture") or {}).get("raw_sha256") != PINNED_BAT609_RAW_SHA256:
        raise AuthorityViolation("BAT-609 season-index raw hash drifted")
    if official_2003_index_is_ready(data_root, repo_root):
        reconstructed_index = reconstruct_official_2003_index(
            repo_root=repo_root, data_root=data_root
        )
        if reconstructed_index["gate"] != bat609:
            raise AuthorityViolation(
                "BAT-609 committed gate does not match independent reconstruction"
            )
        bat609 = reconstructed_index["gate"]
    if official_2003_boxscores_are_ready(data_root):
        reconstructed_box = reconstruct_official_2003_boxscores(
            repo_root=repo_root, data_root=data_root
        )
        if reconstructed_box["gate"] != bat610:
            raise AuthorityViolation(
                "BAT-610 committed gate does not match independent reconstruction"
            )
    bat606 = load_json(
        repo_root
    ...
    allowlist = [
        validate_official_url(str(url)) for url in (bat609.get("box_score_urls") or [])
    ]
    if not allowlist:
        raise AuthorityViolation(
            "BAT-609 reconstruction emitted no official 2003 box URLs"
        )
    path = data_root / CAPTURE_INDEX_RELATIVE
    if not path.is_file():
        raise AuthorityViolation("BAT-610 capture index missing")
    by_url = {
    ...
    for url in allowlist:
        if url not in by_url:
            raise AuthorityViolation(
                f"BAT-610 capture missing official 2003 URL: {url}"
            )
        record = by_url[url]
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    captures = load_2003_captures(repo_root, data_root)
    ...
    payload = {
        ...
        "admissions": {
            ...
            "bat_609_payload": "CONSUMED_INDEX_CAPTURE_ONLY",
            "bat_610_payload": "CONSUMED_CAPTURES_ONLY",
            "gap_005": "OPEN",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2003_STRUCTURED_DOMAINS_GATE",
        "result": PASS_RESULT,
...
def validate_compact_gate(committed: Mapping[str, Any]) -> None:
    ...
    if committed.get("result") not in {
        PASS_RESULT,
        "PARTIAL_OFFICIAL_2003_STRUCTURED_DOMAINS",
    }:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2003 structured-domain reconstruction was required but the data root is not mounted"
        )
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2003 structured-domain gate does not match independent reconstruction"
        )
    payload_path = (
        data_root
