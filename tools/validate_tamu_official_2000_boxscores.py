...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Independently validate official 2000 box-score acquisition."
    )
    parser.add_argument(
    ...
    if not args.validate_only:
        ...
        mutations.append(
            expect_rejection(
                ...
                lambda: _validate(_mutated_gate(gate, authority=authority2)),
            )
        )
        ...
        counts3["rich_structured_games"] = (
            int(counts3.get("rich_structured_games") or 0) + 1
        )
        mutations.append(
            expect_rejection(
                "rich_count_tamper",
        ...
        mutations.append(
            expect_rejection(
                "bat613_rewrite",
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream3)),
            )
        )
        upstream_bat621 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_bat621["bat625_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat625_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_bat621)
                ),
            )
        )
        upstream_bat622 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_bat622["bat622_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat622_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_bat622)
                ),
            )
        )
        upstream_bat615 = json.loads(json.dumps(gate["upstream_identities"]))
        upstream_bat615["bat615_gate_identity"] = "0" * 64
        mutations.append(
            expect_rejection(
                "bat615_rewrite",
                lambda: _validate(
                    _mutated_gate(gate, upstream_identities=upstream_bat615)
                ),
            )
        )
        mutations.append(
        ...
        mutations.append(
            expect_rejection(
                "bat610_rewrite",
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream5)),
            )
        )
        ...
        mutations.append(
            expect_rejection(
                "bat605_rewrite",
                lambda: _validate(_mutated_gate(gate, upstream_identities=upstream4)),
            )
        )
        ...
        mutations.append(
            expect_rejection("selected_season_tamper", lambda: _validate(invented))
        )
        mutations.append(
            expect_rejection(
