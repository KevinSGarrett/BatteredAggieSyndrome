...
from aggie_analytics.data.tamu_official_2003_season_index import (  # noqa: E402
    AuthorityViolation,
    GATE_RELATIVE,
...
OFFICIAL_2003_URL = "https://files.12thman.com/history/football/years/2003.html"
...
FIXTURE_HTML = (
    ...
    "<td><a href='../stats/2003-2004/ta01-asu.html'>Box Score</a></td></tr>"
    "<tr><td>Sep 6</td><td>Utah</td><td>College Station</td><td>W, 28-26</td>"
    "<td><a href='../stats/2003-2004/ta02-utah.html'>Box Score</a></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
AMBIGUOUS_HTML = (
    ...
    "<td><a href='../stats/2003-2004/ta03-unk.html'>Box Score</a></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
class Official2003UrlAndFixtureTests(unittest.TestCase):
    def test_guessed_year_url_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(AuthorityViolation, "guessed|non-official"):
                build_objects(
                    body=FIXTURE_HTML,
                    capture=_capture("https://files.12thman.com/history/football/years/2004.html", FIXTURE_HTML),
                    repo_root=REPO_ROOT,
                    data_root=Path(tmp),
                )
    ...
    def test_fixture_discovers_source_ordered_rows_and_box_urls(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            stored = persist_capture(
                Path(tmp),
                {
                    **_capture(OFFICIAL_2003_URL, FIXTURE_HTML),
            ...
            self.assertEqual(
                ...
                [
                    "https://files.12thman.com/history/football/stats/2003-2004/ta01-asu.html",
                    "https://files.12thman.com/history/football/stats/2003-2004/ta02-utah.html",
                ],
            )
            ...
            self.assertEqual(
                objects["gate"]["upstream_identities"]["bat604_gate_identity"],
                PINNED_BAT604_GATE_IDENTITY,
            )
            ...
            self.assertEqual(objects["gate"]["season"], 2003)
            self.assertEqual(objects["gate"]["jira_key"], "BAT-609")

    def test_missing_box_link_is_counted_not_invented(self) -> None:
        parsed = parse_season_game_rows(
            body=MISSING_BOX_HTML,
            page_url=OFFICIAL_2003_URL,
...
class Compact2003GateTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2003 gate not materialized yet")
    ...
    def test_ci_path_validates_without_external_history_index(self) -> None:
        ...
        self.assertEqual(self.gate["official_index_url"], OFFICIAL_2003_URL)
        self.assertEqual(self.gate["upstream_identities"]["bat604_gate_identity"], PINNED_BAT604_GATE_IDENTITY)
...
@unittest.skipUnless(LAKE_READY, "external BAT-609 2003 capture is not mounted")
class Official2003CaptureTests(unittest.TestCase):
