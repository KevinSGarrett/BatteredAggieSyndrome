"""Acquire and normalize official SRC-014 2001 box scores from the independently reconstructed BAT-621 index."""
...
SCHEMA_VERSION = "aggie.data.tamu_official_2001_boxscores.v1"
CONTRACT_RELATIVE = "configs/tamu_official_2001_boxscore_contract.json"
GATE_RELATIVE = "artifacts/data_lake/tamu_official_2001_boxscore_gate.json"
CONTRACT_ID = "BAT-622-TAMU-OFFICIAL-2001-BOXSCORES-V1"
SOURCE_ID = "SRC-014"
PASS_CLASSIFICATION = "TAMU_SRC014_OFFICIAL_2001_BOXSCORE_CANDIDATE_ONLY"
PASS_RESULT = "PASS_OFFICIAL_2001_BOXSCORES_NORMALIZED"
...
MODULE_RELATIVE = "src/aggie_analytics/data/tamu_official_2001_boxscores.py"
...
def load_source_index(repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    committed = load_json(repo_root / BAT621_GATE_RELATIVE)
    if committed.get("gate_identity") != PINNED_BAT621_GATE_IDENTITY:
        raise AuthorityViolation("BAT-621 gate identity drifted")
    if committed.get("inventory_identity") != INVENTORY_IDENTITY:
        raise AuthorityViolation("BAT-585 inventory identity rewritten")
    if committed.get("official_index_url") != OFFICIAL_2001_INDEX_URL:
        raise AuthorityViolation("guessed or substituted 2001 official URL")
    if committed.get("payload_identity") != PINNED_BAT621_PAYLOAD_IDENTITY:
        raise AuthorityViolation("BAT-621 payload identity rewritten")
    if committed.get("capture_identity") != PINNED_BAT621_CAPTURE_IDENTITY:
        raise AuthorityViolation("BAT-621 capture identity rewritten")
    if committed.get("box_url_identity") != PINNED_BAT621_BOX_URL_IDENTITY:
    ...
    if (committed.get("capture") or {}).get("raw_sha256") != PINNED_BAT621_RAW_SHA256:
    ...
    if official_2001_index_is_ready(data_root, repo_root):
        reconstructed = reconstruct_official_2001_index(
    ...
    return {
        ...
        "season": SEASON,
        "official_index_url": OFFICIAL_2001_INDEX_URL,
        "box_score_urls": urls,
...
def compact_official_2001_capture(
...
def acquire_missing(
    *,
    ...
    for target in targets:
        ...
        captures.append(
            compact_official_2001_capture(stored, source_order=source_order)
        )
        known.add(url)
...
def reconstruct_objects(*, repo_root: Path, data_root: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "artifact_type": "TAMU_OFFICIAL_2001_BOXSCORES",
        "contract_id": CONTRACT_ID,
        "decision_unit": "POST-TASK-SRC014-2001-OFFICIAL-ACQUISITION-001",
        "jira_key": "BAT-622",
        "source_id": SOURCE_ID,
        ...
        | {
            ...
            "bat_621": "CONSUMED_INDEX_CAPTURE_ONLY",
    ...
    gate = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "TAMU_OFFICIAL_2001_BOXSCORE_GATE",
        "result": PASS_RESULT
        if not blocked and counts["normalized_games"] == counts["target_games_total"]
        else "PARTIAL_OFFICIAL_2001_BOXSCORES",
...
def validate_compact_gate(committed: Mapping[str, Any], repo_root: Path) -> None:
    ...
    if committed.get("counts", {}).get("ncaa_contest_ids_created"):
        raise AuthorityViolation("NCAA contest IDs fabricated")
    if committed.get("result") not in {PASS_RESULT, "PARTIAL_OFFICIAL_2001_BOXSCORES"}:
...
def validate_artifact(
    *,
    ...
    if require_rebuild and not ready:
        raise AuthorityViolation(
            "external 2001 box-score reconstruction was required but the data root is not mounted"
        )
    if not ready:
        return {
            "result": "PASS",
    ...
    if committed != expected["gate"]:
        raise AuthorityViolation(
            "committed 2001 box-score gate does not match independent reconstruction"
        )
    payload_path = (
        data_root
