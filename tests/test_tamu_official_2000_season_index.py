...
from aggie_analytics.data.tamu_official_2000_season_index import (  # noqa: E402
    AuthorityViolation,
    GATE_RELATIVE,
...
OFFICIAL_2000_URL = "https://files.12thman.com/history/football/years/2000.html"
...
FIXTURE_HTML = (
    ...
    "<td><a href='../stats/2000-2001/ta01-asu.html'>Box Score</a></td></tr>"
    "<tr><td>Sep 6</td><td>Utah</td><td>College Station</td><td>W, 28-26</td>"
    "<td><a href='../stats/2000-2001/ta02-utah.html'>Box Score</a></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
AMBIGUOUS_HTML = (
    ...
    "<td><a href='../stats/2000-2001/ta03-unk.html'>Box Score</a></td></tr>"
    "</table></body></html>"
).encode("utf-8")
...
class Official2000UrlAndFixtureTests(unittest.TestCase):
    ...
    def test_fixture_discovers_source_ordered_rows_and_box_urls(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            stored = persist_capture(
                Path(tmp),
                {
                    **_capture(OFFICIAL_2000_URL, FIXTURE_HTML),
            ...
            self.assertEqual(
                ...
                [
                    "https://files.12thman.com/history/football/stats/2000-2001/ta01-asu.html",
                    "https://files.12thman.com/history/football/stats/2000-2001/ta02-utah.html",
                ],
            )
            ...
            self.assertEqual(
                objects["gate"]["inventory_identity"], PINNED_INVENTORY_IDENTITY
            )
            self.assertEqual(
            ...
            self.assertEqual(
                objects["gate"]["game_rows"][0]["source_opponent"], "Arkansas State"
            )
            self.assertEqual(objects["gate"]["game_rows"][0]["source_row_order"], 1)
            self.assertIsNone(objects["gate"]["game_rows"][0]["ncaa_contest_id"])
            self.assertEqual(objects["gate"]["season"], 2000)
            self.assertEqual(objects["gate"]["jira_key"], "BAT-625")
            self.assertEqual(
                objects["gate"]["upstream_identities"]["bat621_gate_identity"],
                PINNED_BAT621_GATE_IDENTITY,
            )
            self.assertEqual(
...
class Compact2000GateTests(unittest.TestCase):
    def setUp(self) -> None:
        path = REPO_ROOT / GATE_RELATIVE
        if not path.is_file():
            self.skipTest("2000 gate not materialized yet")
    ...
    def test_ci_path_validates_without_external_history_index(self) -> None:
        ...
        self.assertEqual(self.gate["official_index_url"], OFFICIAL_2000_URL)
        self.assertEqual(
            self.gate["upstream_identities"]["bat609_gate_identity"],
            PINNED_BAT609_GATE_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat613_gate_identity"],
            PINNED_BAT613_GATE_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat621_gate_identity"],
            PINNED_BAT621_GATE_IDENTITY,
        )
    ...
    def test_changed_code_with_stale_code_identity(self) -> None:
        with mock.patch(
            "aggie_analytics.data.tamu_official_2000_season_index.compute_code_identity",
            return_value="0" * 64,
        ):
            with self.assertRaisesRegex(AuthorityViolation, "stale code identity"):
                validate_artifact(
    ...
    def test_bat613_identity_rewrite_fails_without_rebuild(self) -> None:
        ...
        with self.assertRaisesRegex(
            AuthorityViolation, "BAT-613 2002 gate identity rewritten"
        ):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
    ...
    def test_bat621_identity_rewrite_fails_without_rebuild(self) -> None:
        upstream = json.loads(json.dumps(self.gate["upstream_identities"]))
        upstream["bat621_gate_identity"] = "0" * 64
        with self.assertRaisesRegex(
            AuthorityViolation, "BAT-621 2001 gate identity rewritten"
        ):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
...
@unittest.skipUnless(LAKE_READY, "external BAT-625 2000 capture is not mounted")
class Official2000CaptureTests(unittest.TestCase):
