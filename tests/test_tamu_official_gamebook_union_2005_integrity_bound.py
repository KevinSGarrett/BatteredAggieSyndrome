...
class CompactIntegrityBoundUnionTests(unittest.TestCase):
    ...
    def test_committed_counts_predecessor_and_no_new_games(self) -> None:
        ...
        self.assertEqual(self.gate["predecessor_union_identity"], PINNED_BAT602_UNION_IDENTITY)
        self.assertEqual(self.gate["predecessor_gate_identity"], PINNED_BAT602_GATE_IDENTITY)
        ...
        self.assertEqual(self.gate["counts"]["official_2005_preserved"], OFFICIAL_2005_EXPECTED)
        self.assertEqual(self.gate["counts"]["official_2005_revalidated"], OFFICIAL_2005_EXPECTED)
        self.assertEqual(self.gate["counts"]["prior_261_union_games_preserved"], PRIOR_UNION_CAPTURED_GAMES)
        ...
        self.assertEqual(self.gate["upstream_identities"]["bat600_dataset_identity"], PINNED_BAT600_DATASET_IDENTITY)
        self.assertEqual(self.gate["upstream_identities"]["bat601_payload_identity"], PINNED_BAT601_PAYLOAD_IDENTITY)
        self.assertEqual(self.gate["recomputed_upstream"]["bat600_dataset_identity"], PINNED_BAT600_DATASET_IDENTITY)
        self.assertEqual(self.gate["recomputed_upstream"]["bat601_payload_identity"], PINNED_BAT601_PAYLOAD_IDENTITY)
        ...
        self.assertTrue(all(item.get("parent_url") == OFFICIAL_2005_INDEX_URL for item in self.gate["admitted_official_2005_games"]))
        ...
        self.assertTrue(all(item.get("availability") == "NOT_ESTABLISHED" for item in self.gate["admitted_official_2005_games"]))
        ...
        self.assertFalse(self.gate["scientific_nonclaims"]["new_historical_coverage_claimed"])
    ...
    def test_prior_identities_are_not_rewritten(self) -> None:
        ...
        self.assertEqual(bat600["acquisition_identity"], PINNED_BAT600_ACQUISITION_IDENTITY)
    ...
    def test_parent_url_fallback_is_forbidden(self) -> None:
        with self.assertRaisesRegex(AuthorityViolation, "parent_url missing"):
            compact_official_2005({"url": "https://example.invalid/box"}, OFFICIAL_2005_INDEX_URL)
        with self.assertRaisesRegex(AuthorityViolation, "does not match BAT-599"):
            compact_official_2005(
                {"url": "https://example.invalid/box", "parent_url": "https://files.12thman.com/history/football/years/2004.html"},
                OFFICIAL_2005_INDEX_URL,
            )
    ...
    def test_ncaa_id_and_rejected_url_fail(self) -> None:
        ...
        with self.assertRaisesRegex(AuthorityViolation, "rejected"):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, gate=_mutated(self.gate, enriched_official_games=games), require_rebuild=False)
...
class IntegrityBoundReconstructionAndTamperTests(unittest.TestCase):
    ...
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(result["recomputed_upstream"]["bat600_dataset_identity"], PINNED_BAT600_DATASET_IDENTITY)
        self.assertEqual(result["recomputed_upstream"]["bat601_payload_identity"], PINNED_BAT601_PAYLOAD_IDENTITY)
    ...
    def test_bat600_identity_recompute_matches_pins(self) -> None:
        ...
        self.assertEqual(recomputed["acquisition_identity"], PINNED_BAT600_ACQUISITION_IDENTITY)
        self.assertEqual(recomputed["games_identity"], PINNED_BAT600_GAMES_IDENTITY)
        validated = validate_bat600_external_payload(repo_root=REPO_ROOT, data_root=DATA_ROOT)
        self.assertEqual(validated["identities"], recomputed)
        self.assertTrue(all(game["parent_url"] == OFFICIAL_2005_INDEX_URL for game in validated["games"]))

    def test_bat601_row_counts_recompute_from_serialized_rows(self) -> None:
        validated = validate_bat601_external_payload(repo_root=REPO_ROOT, data_root=DATA_ROOT)
    ...
    def test_bat600_opponent_score_sha_parent_and_membership_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=opponent)
        score = _copy(self.bat600)
        score["games"][0]["tamu_points"] = 99
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=score)
        sha = _copy(self.bat600)
        sha["games"][0]["source_sha256"] = "0" * 64
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=sha)
        missing = _copy(self.bat600)
        missing["games"][0].pop("parent_url", None)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=missing)
        substituted = _copy(self.bat600)
        substituted["games"][0]["parent_url"] = "https://files.12thman.com/history/football/years/2004.html"
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=substituted)
        captures = _copy(self.bat600)
        captures["captures"] = captures["captures"][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=captures)
        conflicts = _copy(self.bat600)
        conflicts["conflicts"] = conflicts["conflicts"][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=conflicts)
    ...
    def test_bat601_row_and_coverage_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=changed)
        removed = _copy(self.bat601)
        removed["rows"][0] = removed["rows"][0][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=removed)
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=added)
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=present_zero)
        parser = _copy(self.bat601)
        parser["games"][0]["parser_identity"] = "forged.parser.v0"
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=parser)
        availability = _copy(self.bat601)
        availability["availability_claim"] = True
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat601_payload=availability)
        compact = _copy(load_json(REPO_ROOT / "artifacts/data_lake/tamu_official_2005_structured_domains_gate.json")["games"])
    ...
    def test_coordinated_tamper_plus_recomputed_outer_identity_fails(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(repo_root=REPO_ROOT, data_root=DATA_ROOT, bat600_payload=bat600, bat601_payload=bat601)
