...
| Reconstruction SHA | `6c7e99f06d8b5b233bbd3cd2af7440131c3ae767` recorded in `CYCLE30_PREFLIGHT_AND_PRESERVATION.json` `head_sha` |
...
- **stats.ncaa.org 403** for contests 6602874 / 6620581 / 6594400 — ncaa.com Final is not stats Final; SMU stays `AWAITING_OFFICIAL_FINAL` on the stats identity; CFBD 27–24 is not NCAA official Final; expired T-90 is not recaptured as on-time.
- **NCAA discontinued census** — 0 NCAA JSON rows; Wikipedia names are not an NCAA census.
- **Availability player joins** — 14 policy routes attempted; 0 player names on those pages/PDFs; no report = UNKNOWN, not healthy.
- **10 UNKNOWN OC/DC cells** listed above — official titles do not name that coordinator; pass/run-game coordinator is not OC/DC.
...
- **Pre-2013 FCS parent join / canceled-forfeit audit** — CFBD rows exist; parent identity semantic join and forfeit audit remain manager/remaining work.
