"""Atomic acquisition states, request identity, and official-final admission.

Transport success, upstream success, semantic availability, contest identity,
terminal status, and score admission are distinct states.
"""
...
from aggie_analytics.cycle29.temporal import TemporalError, parse_aware_utc
...
def contest_scoped_terminal(
    *,
    ...
    if not _FINAL_TOKEN.search(contest_window):
        return "NOT_TERMINAL"
    return TERMINAL_STATUS_ESTABLISHED


def _contest_window(page_text: str, contest_id: str) -> str | None:
    idx = page_text.find(contest_id)
    if idx < 0:
        return None
    start = max(0, idx - 400)
