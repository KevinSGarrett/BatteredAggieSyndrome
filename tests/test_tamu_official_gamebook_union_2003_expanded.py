...
class CompactExpanded2003UnionTests(unittest.TestCase):
    ...
    def test_committed_counts_predecessor_and_2003_admissions(self) -> None:
        ...
        self.assertEqual(
            self.gate["predecessor_union_identity"], PINNED_BAT608_UNION_IDENTITY
        )
        self.assertEqual(
            self.gate["predecessor_gate_identity"], PINNED_BAT608_GATE_IDENTITY
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.gate["counts"]["official_2003_admitted"], OFFICIAL_2003_EXPECTED
        )
        self.assertEqual(self.gate["counts"]["official_2003_rejected"], 0)
        self.assertEqual(
            self.gate["counts"]["predecessor_273_union_games_preserved"],
            PRIOR_UNION_CAPTURED_GAMES,
        )
        ...
        self.assertEqual(
            self.gate["upstream_identities"]["bat610_dataset_identity"],
            PINNED_BAT610_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["upstream_identities"]["bat611_payload_identity"],
            PINNED_BAT611_PAYLOAD_IDENTITY,
        )
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat610_dataset_identity"],
            PINNED_BAT610_DATASET_IDENTITY,
        )
        self.assertEqual(
            self.gate["recomputed_upstream"]["bat611_payload_identity"],
            PINNED_BAT611_PAYLOAD_IDENTITY,
        )
        ...
        self.assertTrue(
            all(
                item.get("parent_url") == OFFICIAL_2003_INDEX_URL
                for item in self.gate["admitted_official_2003_games"]
            )
        )
        self.assertTrue(
            all(
        ...
        self.assertEqual(
            self.gate["selected_seasons"], [2009, 2008, 2007, 2006, 2005, 2004, 2003]
        )
    ...
    def test_prior_identities_are_not_rewritten(self) -> None:
        ...
        self.assertEqual(
            bat610["acquisition_identity"], PINNED_BAT610_ACQUISITION_IDENTITY
        )
    ...
    def test_parent_url_fallback_is_forbidden(self) -> None:
        with self.assertRaisesRegex(AuthorityViolation, "parent_url missing"):
            compact_official_2003(
                {"url": "https://example.invalid/box"}, OFFICIAL_2003_INDEX_URL
            )
...
@unittest.skipUnless(
    LAKE_READY, "external BAT-608/BAT-610/BAT-611 payloads are not mounted"
)
class Expanded2003ReconstructionAndTamperTests(unittest.TestCase):
    ...
    def test_committed_gate_reconstructs(self) -> None:
        ...
        self.assertEqual(
            result["recomputed_upstream"]["bat610_dataset_identity"],
            PINNED_BAT610_DATASET_IDENTITY,
        )
        self.assertEqual(
            result["recomputed_upstream"]["bat611_payload_identity"],
            PINNED_BAT611_PAYLOAD_IDENTITY,
        )
    ...
    def test_bat610_identity_recompute_matches_pins(self) -> None:
        ...
        self.assertEqual(
            recomputed["acquisition_identity"], PINNED_BAT610_ACQUISITION_IDENTITY
        )
    ...
    def test_bat610_opponent_score_sha_parent_and_membership_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=opponent
            )
        score = _copy(self.bat610)
        score["games"][0]["tamu_points"] = 99
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=score
            )
        sha = _copy(self.bat610)
        sha["games"][0]["source_sha256"] = "0" * 64
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=sha
            )
        missing = _copy(self.bat610)
        missing["games"][0].pop("parent_url", None)
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=missing
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=substituted
            )
        captures = _copy(self.bat610)
        captures["captures"] = captures["captures"][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=captures
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat610_payload=name_only
            )
    ...
    def test_bat611_row_and_coverage_tampers_fail(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=changed
            )
        removed = _copy(self.bat611)
        removed["rows"][0] = removed["rows"][0][1:]
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=removed
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=added
            )
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=present_zero
            )
        parser = _copy(self.bat611)
        parser["games"][0]["parser_identity"] = "forged.parser.v0"
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=parser
            )
        availability = _copy(self.bat611)
        availability["availability_claim"] = True
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT, data_root=DATA_ROOT, bat611_payload=availability
            )
        compact = _copy(
            load_json(
                REPO_ROOT
    ...
    def test_coordinated_tamper_plus_recomputed_outer_identity_fails(self) -> None:
        ...
        with self.assertRaises(AuthorityViolation):
            validate_artifact(
                repo_root=REPO_ROOT,
                data_root=DATA_ROOT,
                bat610_payload=bat610,
                bat611_payload=bat611,
            )
