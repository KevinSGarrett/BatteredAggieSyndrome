...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Independently consume and validate the 2000-2009 official structured row corpus."
    )
    parser.add_argument(
    ...
    consumed = consume_corpus(
        data_root=data_root, dataset_identity=validated["dataset_identity"]
    )
    mutations = [
        expect_rejection(
        ...
        json.dumps(
            {
                ...
                "consumed_row_counts": {
                    domain: len(consumed[domain]) for domain in CHILD_DOMAINS
                },
