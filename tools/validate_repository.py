...
def main() -> int:
    ...
    if args.strict:
        ...
        if box_2007_gate.is_file() and box_2007_module.is_file():
            ...
            if spec is None or spec.loader is None:
                findings.append(
                    type("F", (), {"kind": "tamu_official_2007_boxscores", "path": "src/aggie_analytics/data/tamu_official_2007_boxscores.py", "detail": "unable to load official 2007 box-score validator"})()
                )
            else:
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
