...
def main() -> int:
    parser = argparse.ArgumentParser(description="Independently validate the 2002-expanded official union.")
    ...
    if not args.validate_only:
        ...
        bat615 = _copy(objects["bat610"]["payload"])
        bat617 = _copy(objects["bat611"]["payload"])
        official_index_urls = [str(item.get("url") or "") for item in objects["bat610"]["games"]]
        ...
        def _validate_payloads(
            bat615_payload: dict[str, Any] | None = None,
            bat617_payload: dict[str, Any] | None = None,
        ) -> Any:
            return validate_artifact(
                repo_root=repo_root,
                data_root=data_root,
                require_rebuild=True,
                bat615_payload=bat615_payload,
                bat617_payload=bat617_payload,
            )
        ...
        mutations.append(expect_rejection("oklahoma_unmatched_admitted", lambda: _validate(_mutated_gate(gate, enriched_official_games=oklahoma))))
        ...
        mutations.append(expect_rejection("bat612_union_rewritten", lambda: _validate(_mutated_gate(predecessor))))
        bat608 = _copy(gate["upstream_identities"])
        bat608["bat608_union_identity"] = "0" * 64
        mutations.append(expect_rejection("bat608_union_rewritten", lambda: _validate(_mutated_gate(gate, upstream_identities=bat608))))
        ...
        opponent = _copy(bat615)
        opponent["games"][0]["opponent_candidate"] = "FORGED OPPONENT"
        mutations.append(expect_rejection("bat615_opponent_changed_identities_unchanged", lambda: _validate_payloads(bat615_payload=opponent)))
        score = _copy(bat615)
        score["games"][0]["tamu_points"] = 99
        mutations.append(expect_rejection("bat615_score_changed", lambda: _validate_payloads(bat615_payload=score)))
        source_sha = _copy(bat615)
        source_sha["games"][0]["source_sha256"] = "0" * 64
        mutations.append(expect_rejection("bat615_source_sha_changed", lambda: _validate_payloads(bat615_payload=source_sha)))
        parent_removed = _copy(bat615)
        parent_removed["games"][0].pop("parent_url", None)
        mutations.append(expect_rejection("bat615_parent_url_removed", lambda: _validate_payloads(bat615_payload=parent_removed)))
        parent_sub = _copy(bat615)
        parent_sub["games"][0]["parent_url"] = "https://files.12thman.com/history/football/years/2003.html"
        mutations.append(expect_rejection("bat615_parent_url_substituted", lambda: _validate_payloads(bat615_payload=parent_sub)))
        captures = _copy(bat615)
        captures["captures"] = captures["captures"][1:]
        mutations.append(expect_rejection("bat615_capture_membership_changed", lambda: _validate_payloads(bat615_payload=captures)))
        name_only = _copy(bat615)
        name_only["games"][0]["canonical_game_match_status"] = "MATCHED_OPPONENT_NAME_ONLY"
        mutations.append(expect_rejection("opponent_name_only_admission", lambda: _validate_payloads(bat615_payload=name_only)))

        row_changed = _copy(bat617)
        if row_changed["rows"][0]:
            row_changed["rows"][0][0]["cells"] = ["FORGED"]
        mutations.append(expect_rejection("bat617_row_changed", lambda: _validate_payloads(bat617_payload=row_changed)))
        row_removed = _copy(bat617)
        row_removed["rows"][0] = row_removed["rows"][0][1:]
        mutations.append(expect_rejection("bat617_row_removed", lambda: _validate_payloads(bat617_payload=row_removed)))
        row_added = _copy(bat617)
        ...
        mutations.append(expect_rejection("bat617_row_added", lambda: _validate_payloads(bat617_payload=row_added)))
        present_zero = _copy(bat617)
        present_zero["rows"][0] = []
        present_zero["games"][0]["team_statistics"] = []
        present_zero["games"][0]["domain_coverage"]["team_statistics"] = "PRESENT"
        mutations.append(expect_rejection("bat617_present_with_zero_rows", lambda: _validate_payloads(bat617_payload=present_zero)))
        parser = _copy(bat617)
        parser["games"][0]["parser_identity"] = "forged.parser.v0"
        mutations.append(expect_rejection("bat617_parser_identity_changed", lambda: _validate_payloads(bat617_payload=parser)))
        availability = _copy(bat617)
        availability["availability_claim"] = True
        mutations.append(expect_rejection("bat617_availability_promoted", lambda: _validate_payloads(bat617_payload=availability)))

        compact = _copy((load_json(repo_root / "artifacts/data_lake/tamu_official_2002_structured_domains_gate.json")).get("games") or [])
        compact[0]["row_counts"]["team_statistics"] = int(compact[0]["row_counts"]["team_statistics"]) + 1
        mutations.append(
            expect_rejection(
                "bat617_gate_row_count_changed",
                lambda: validate_bat617_external_payload(
                    repo_root=repo_root,
                    data_root=data_root,
                    payload=bat617,
        ...
        coordinated = _copy(bat615)
        coordinated["games"][0]["opponent_candidate"] = "COORDINATED TAMPER"
        coordinated.update(recompute_bat615_identities(coordinated))
        coordinated_617 = _copy(bat617)
        if coordinated_617["rows"][0]:
            coordinated_617["rows"][0][0]["source_label"] = "COORDINATED TAMPER"
        coordinated_617["payload_identity"] = recompute_bat617_payload_identity(coordinated_617)
        mutations.append(
            expect_rejection(
                "coordinated_external_tamper_plus_recomputed_outer_identity",
                lambda: _validate_payloads(bat615_payload=coordinated, bat617_payload=coordinated_617),
            )
        )
        mutations.append(
            expect_rejection(
                "parent_url_fallback_direct",
                lambda: compact_official_2002({"url": "https://example.invalid/box"}, OFFICIAL_2002_INDEX_URL),
            )
        )
        if gate["upstream_identities"]["bat617_payload_identity"] != PINNED_BAT617_PAYLOAD_IDENTITY:
            raise AssertionError("successor must bind the pinned BAT-617 payload identity")
        if gate["predecessor_union_identity"] != PINNED_BAT612_UNION_IDENTITY:
            raise AssertionError("successor must pin the BAT-612 predecessor union")
        ...
        if any(not item.get("parent_url") for item in gate["admitted_official_2002_games"]):
            raise AssertionError("successor admitted a 2002 game without parent_url")
